  ...
  defaultCallbacks: {
    ...
    onStepFinish: (step: StepResult) => {
      ...
      if (step.toolCalls.length > 0) {
        console.log(`  Tool calls: ${step.toolCalls.map((tc) => tc.name).join(", ")}`);
      }
      console.log(
...
async function conversationExample() {
  ...
  console.log("\nConversation history length:", agent.getConversationHistory().length);
}
...
async function registryExample() {
  ...
  console.log(
    "Math tools:",
    registry.getGroup("math").map((t) => t.name),
  );
  console.log(
    "Utility tools:",
