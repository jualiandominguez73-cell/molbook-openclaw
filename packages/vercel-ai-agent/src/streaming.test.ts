...
describe("Stream utilities", () => {
  ...
  describe("filterStreamByType", () => {
    ...
    it("should allow multiple types", async () => {
      const filtered = filterStreamByType(createTestStream(), ["text-delta", "tool-call"]);
  ...
  describe("batchTextDeltas", () => {
    it("should batch text deltas", async () => {
      ...
      const totalText = textChunks.reduce((acc, c) => acc + (c.textDelta || ""), "");
  ...
  describe("addTimingInfo", () => {
    it("should add timestamp and elapsed time", async () => {
      ...
      expect(chunks[chunks.length - 1].elapsed).toBeGreaterThanOrEqual(chunks[0].elapsed);
    });
  });
