...
export const dateTimeTool = tool({
  ...
  inputSchema: z.object({
    timezone: z
      .string()
      .optional()
      .describe('IANA timezone name (e.g., "America/New_York", "Europe/London"). Defaults to UTC.'),
    format: z
...
export const jsonTool = tool({
  ...
  inputSchema: z.object({
    action: z.enum(["parse", "stringify", "validate"]).describe("The action to perform"),
    input: z.string().describe("JSON string to parse/validate, or object to stringify"),
    pretty: z.boolean().optional().describe("Whether to pretty-print the output (for stringify)"),
  }),
    ...
    switch (action) {
      ...
      case "stringify":
        try {
          ...
          const stringified = pretty ? JSON.stringify(obj, null, 2) : JSON.stringify(obj);
...
export const stringTool = tool({
  ...
  inputSchema: z.object({
    ...
    arg: z
      .string()
      .optional()
      .describe("Additional argument (delimiter for split/join, search string for replace)"),
...
export const waitTool = tool({
  description: "Wait for a specified number of milliseconds. Useful for testing or rate limiting.",
  inputSchema: z.object({
    milliseconds: z
      .number()
      .min(0)
