...
export class ConversationManager {
   ...
   */
  addToolResultMessage(sessionId: string, toolResult: ToolResult): ConversationMessage {
    return this.addMessage(sessionId, {
      role: "tool",
...
export function truncateToTokenBudget(
  ...
  const systemMessages = preserveSystemMessages ? messages.filter((m) => m.role === "system") : [];
  const otherMessages = preserveSystemMessages
    ? messages.filter((m) => m.role !== "system")
    : [...messages];

  // Calculate tokens for system messages
  let usedTokens = systemMessages.reduce((sum, m) => sum + estimateTokenCount(m), 0);
  ...
  usedTokens += preservedMessages.reduce((sum, m) => sum + estimateTokenCount(m), 0);
