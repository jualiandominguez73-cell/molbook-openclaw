...
describe("ConversationManager", () => {
  ...
  describe("message management", () => {
    ...
    it("should add assistant message with tool calls", () => {
      const toolCalls = [{ id: "tc1", name: "test_tool", arguments: { input: "test" } }];
      const message = manager.addAssistantMessage(sessionId, "Using tool...", toolCalls);
      expect(message.toolCalls).toEqual(toolCalls);
    });

    it("should add system message", () => {
      const message = manager.addSystemMessage(sessionId, "You are a helpful assistant");
...
describe("truncateToTokenBudget", () => {
  ...
  it("should respect token budget", () => {
    const messages = Array.from({ length: 10 }, () => createMessage("x".repeat(100)));
