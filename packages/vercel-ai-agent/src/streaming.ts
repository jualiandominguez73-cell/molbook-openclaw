    ...
    switch (chunk.type) {
      ...
      case "step-finish":
        if (chunk.stepResult) {
          ...
          this.state.usage.completionTokens += chunk.stepResult.usage.completionTokens;
 ...
 */
export async function collectStreamText(source: AsyncIterable<StreamChunk>): Promise<string> {
  let text = "";
  for await (const chunk of source) {
...
export async function* batchTextDeltas(
  ...
  for await (const chunk of source) {
    if (chunk.type === "text-delta" && chunk.textDelta) {
      ...
      const shouldYield = textBuffer.length >= batchSize || Date.now() - lastYield >= maxWaitMs;
 ...
 */
export function toReadableStream(source: AsyncIterable<StreamChunk>): ReadableStream<StreamChunk> {
