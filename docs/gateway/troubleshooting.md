---
summary: "Quick troubleshooting guide for common Clawdbrain failures"
read_when:
  - Investigating runtime issues or failures
---
# Troubleshooting 🔧

When Clawdbrain misbehaves, here's how to fix it.
...
|---|---|---|
| `clawdbrain status` | Local summary: OS + update, gateway reachability/mode, service, agents/sessions, provider config state | First check, quick overview |
| `clawdbrain status --all` | Full local diagnosis (read-only, pasteable, safe-ish) incl. log tail | When you need to share a debug report |
| `clawdbrain status --deep` | Runs gateway health checks (incl. provider probes; requires reachable gateway) | When “configured” doesn’t mean “working” |
| `clawdbrain gateway probe` | Gateway discovery + reachability (local + remote targets) | When you suspect you’re probing the wrong gateway |
| `clawdbrain channels status --probe` | Asks the running gateway for channel status (and optionally probes) | When gateway is reachable but channels misbehave |
| `clawdbrain gateway status` | Supervisor state (launchd/systemd/schtasks), runtime PID/exit, last gateway error | When the service “looks loaded” but nothing runs |
| `clawdbrain logs --follow` | Live logs (best signal for runtime issues) | When you need the actual failure reason |

**Sharing output:** prefer `clawdbrain status --all` (it redacts tokens). If you paste `clawdbrain status`, consider setting `CLAWDBRAIN_SHOW_SECRETS=0` first (token previews).
...
- Or paste a setup-token on the **gateway host**:
  ```bash
  clawdbrain models auth setup-token --provider anthropic
  ```
...
```bash
clawdbrain models auth paste-token --provider anthropic
clawdbrain models status
```
...
**Logs:**
- Preferred: `clawdbrain logs --follow`
- File logs (always): `/tmp/clawdbrain/clawdbrain-YYYY-MM-DD.log` (or your configured `logging.file`)
- macOS LaunchAgent (if installed): `$CLAWDBRAIN_STATE_DIR/logs/gateway.log` and `gateway.err.log`
- Linux systemd (if installed): `journalctl --user -u clawdbrain-gateway[-<profile>].service -n 200 --no-pager`
...
- Run the wizard and set the Gateway run mode to **Local**:
  ```bash
  clawdbrain configure
  ```
- Or set it directly:
  ```bash
  clawdbrain config set gateway.mode local
  ```

**If you meant to run a remote Gateway instead:**
- Set a remote URL and keep `gateway.mode=remote`:
  ```bash
  clawdbrain config set gateway.mode remote
  clawdbrain config set gateway.remote.url "wss://gateway.example.com"
  ```

**Ad-hoc/dev only:** pass `--allow-unconfigured` to start the gateway without
`gateway.mode=local`.

**No config file yet?** Run `clawdbrain setup` to create a starter config, then rerun
the gateway.
...
variables like `DISPLAY` should live in `~/.clawdbrain/.env` (loaded early by the
gateway).
...
service was installed with Bun or a version-managed Node path, run `clawdbrain doctor`
...
- then run `clawdbrain sandbox recreate --agent <id>` (or `--all`)
...
**Check:**
- `gateway.mode` must be `local` for `clawdbrain gateway` and the service.
- If you set `gateway.mode=remote`, the **CLI defaults** to a remote URL. The service can still be running locally, but your CLI may be probing the wrong place. Use `clawdbrain gateway status` to see the service’s resolved port + probe target (or pass `--url`).
- `clawdbrain gateway status` and `clawdbrain doctor` surface the **last gateway error** from logs when the service looks running but the port is closed.
- Non-loopback binds (`lan`/`tailnet`/`custom`, or `auto` when loopback is unavailable) require auth:
  `gateway.auth.token` (or `CLAWDBRAIN_GATEWAY_TOKEN`).
- `gateway.remote.token` is for remote CLI calls only; it does **not** enable local auth.
- `gateway.token` is ignored; use `gateway.auth.token`.

**If `clawdbrain gateway status` shows a config mismatch**
...
- Fix: rerun `clawdbrain gateway install --force` from the same `--profile` / `CLAWDBRAIN_STATE_DIR` you want the service to use.

**If `clawdbrain gateway status` reports service config issues**
- The supervisor config (launchd/systemd/schtasks) is missing current defaults.
- Fix: run `clawdbrain doctor` to update it (or `clawdbrain gateway install --force` for a full rewrite).
...
- Fix: set `gateway.auth.mode` + `gateway.auth.token` (or export `CLAWDBRAIN_GATEWAY_TOKEN`) and restart the service.

**If `clawdbrain gateway status` says `bind=tailnet` but no tailnet interface was found**
...
If you upgraded from older installs, you might still have `~/clawdbrain` on disk.
...
Symptoms: `pwd` or file tools show `~/.clawdbrain/sandboxes/...` even though you
...
Clawdbrain intentionally rejects **older/insecure models** (especially those more
...
- If you’re unsure which models are available, run `clawdbrain models list` or
  `clawdbrain models scan` and choose a supported one.
...
grep -n "agents\\|groupChat\\|mentionPatterns\\|channels\\.whatsapp\\.groups\\|channels\\.telegram\\.groups\\|channels\\.imessage\\.groups\\|channels\\.discord\\.guilds" \
  "${CLAWDBRAIN_CONFIG_PATH:-$HOME/.clawdbrain/clawdbrain.json}"
```
...
tail -f "$(ls -t /tmp/clawdbrain/clawdbrain-*.log | head -1)" | grep "blocked\\|skip\\|unauthorized"
```
...
```bash
clawdbrain pairing list <channel>
```
...
```bash
clawdbrain logs --follow | grep "pairing request"
```
...
```bash
ls -la ~/.clawdbrain/agents/<agentId>/sessions/
```
...
clawdbrain logs --limit 200 | grep "connection\\|disconnect\\|logout"
```
...
```bash
clawdbrain gateway --verbose
```

If you’re logged out / unlinked:

```bash
clawdbrain channels logout
trash "${CLAWDBRAIN_STATE_DIR:-$HOME/.clawdbrain}/credentials" # if logout can't cleanly remove everything
clawdbrain channels login --verbose       # re-scan QR
```
...
```bash
grep "media\\|fetch\\|download" "$(ls -t /tmp/clawdbrain/clawdbrain-*.log | head -1)" | tail -20
```

### High Memory Usage

Clawdbrain keeps conversation history in memory.
...
Clawdbrain now refuses to start when the config contains unknown keys, malformed values, or invalid types.
...
Notes:
- `clawdbrain doctor` reports every invalid entry.
- `clawdbrain doctor --fix` applies migrations/repairs and rewrites the config.
- Diagnostic commands like `clawdbrain logs`, `clawdbrain health`, `clawdbrain status`, `clawdbrain gateway status`, and `clawdbrain gateway probe` still run even if the config is invalid.

### “All models failed” — what should I check first?

- **Credentials** present for the provider(s) being tried (auth profiles + env vars).
- **Model routing**: confirm `agents.defaults.model.primary` and fallbacks are models you can access.
- **Gateway logs** in `/tmp/clawdbrain/…` for the exact provider error.
- **Model status**: use `/model status` (chat) or `clawdbrain models status` (CLI).
...
```bash
curl -fsSL https://clawdbrain.bot/install.sh | bash -s -- --install-method git --no-onboard
```
...
5) Run `clawdbrain channels status --probe` for audit hints.
...
endpoint accepts a strict subset of JSON Schema. Clawdbrain scrubs/normalizes tool
...
```bash
tccutil reset All com.clawdbrain.mac.debug
```

**Fix 2: Force New Bundle ID**
If resetting doesn't work, change the `BUNDLE_ID` in [`scripts/package-mac-app.sh`](https://github.com/clawdbrain/clawdbrain/blob/main/scripts/package-mac-app.sh) (e.g., add a `.test` suffix) and rebuild. This forces macOS to treat it as a new app.
...
# Or: launchctl bootout gui/$UID/com.clawdbrain.gateway (replace with com.clawdbrain.<profile> if needed)
```
...
Ensure the global `clawdbrain` CLI is installed and matches the app version:
```bash
clawdbrain --version
...
#   ${CLAWDBRAIN_CONFIG_PATH:-$HOME/.clawdbrain/clawdbrain.json} -> { logging: { level: "trace" } }
#
...
| Log | Location |
|-----|----------|
| Gateway file logs (structured) | `/tmp/clawdbrain/clawdbrain-YYYY-MM-DD.log` (or `logging.file`) |
| Gateway service logs (supervisor) | macOS: `$CLAWDBRAIN_STATE_DIR/logs/gateway.log` + `gateway.err.log` (default: `~/.clawdbrain/logs/...`; profiles use `~/.clawdbrain-<profile>/logs/...`)<br />Linux: `journalctl --user -u clawdbrain-gateway[-<profile>].service -n 200 --no-pager`<br />Windows: `schtasks /Query /TN "Clawdbrain Gateway (<profile>)" /V /FO LIST` |
| Session files | `$CLAWDBRAIN_STATE_DIR/agents/<agentId>/sessions/` |
| Media cache | `$CLAWDBRAIN_STATE_DIR/media/` |
| Credentials | `$CLAWDBRAIN_STATE_DIR/credentials/` |
...
clawdbrain gateway status --deep
...
tail -20 /tmp/clawdbrain/clawdbrain-*.log
```
...
trash "${CLAWDBRAIN_STATE_DIR:-$HOME/.clawdbrain}"
clawdbrain channels login         # re-pair WhatsApp
clawdbrain gateway restart           # or: clawdbrain gateway
```
...
1. Check logs first: `/tmp/clawdbrain/` (default: `clawdbrain-YYYY-MM-DD.log`, or your configured `logging.file`)
