...
- During heartbeat runs, Clawdbrain treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `clawdbrain sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Clawdbrain skips the heartbeat run entirely (no model call).
...
```bash
clawdbrain system event --text "Check for urgent follow-ups" --mode now
```
