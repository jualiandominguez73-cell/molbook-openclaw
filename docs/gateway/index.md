...
- Replaces the legacy `gateway` command. CLI entry point: `clawdbrain gateway`.
- Runs until stopped; exits non-zero on fatal errors so the supervisor restarts it.

## How to run (local)
```bash
clawdbrain gateway --port 18789
# for full debug/trace logs in stdio:
clawdbrain gateway --port 18789 --verbose
# if the port is busy, terminate listeners then start:
clawdbrain gateway --force
# dev loop (auto-reload on TS changes):
pnpm gateway:watch
```
- Config hot reload watches `~/.clawdbrain/clawdbrain.json` (or `CLAWDBRAIN_CONFIG_PATH`).
...
- Starts a Canvas file server by default on `canvasHost.port` (default `18793`), serving `http://<gateway-host>:18793/__clawdbrain__/canvas/` from `~/clawd/canvas`. Disable with `canvasHost.enabled=false` or `CLAWDBRAIN_SKIP_CANVAS_HOST=1`.
...
- Gateway auth is required by default: set `gateway.auth.token` (or `CLAWDBRAIN_GATEWAY_TOKEN`) or `gateway.auth.password`. Clients must send `connect.params.auth.token/password` unless using Tailscale Serve identity.
- The wizard now generates a token by default, even on loopback.
- Port precedence: `--port` > `CLAWDBRAIN_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- macOS: `com.clawdbrain.<profile>`
- Linux: `clawdbrain-gateway-<profile>.service`
- Windows: `Clawdbrain Gateway (<profile>)`

Install metadata is embedded in the service config:
- `CLAWDBRAIN_SERVICE_MARKER=clawdbrain`
- `CLAWDBRAIN_SERVICE_KIND=gateway`
- `CLAWDBRAIN_SERVICE_VERSION=<version>`
...
```bash
clawdbrain --dev setup
clawdbrain --dev gateway --allow-unconfigured
# then target the dev instance:
clawdbrain --dev status
clawdbrain --dev health
```

Defaults (can be overridden via env/flags/config):
- `CLAWDBRAIN_STATE_DIR=~/.clawdbrain-dev`
- `CLAWDBRAIN_CONFIG_PATH=~/.clawdbrain-dev/clawdbrain.json`
- `CLAWDBRAIN_GATEWAY_PORT=19001` (Gateway WS + HTTP)
...
- Base port = `gateway.port` (or `CLAWDBRAIN_GATEWAY_PORT` / `--port`)
- browser control service port = base + 2 (loopback only)
- `canvasHost.port = base + 4` (or `CLAWDBRAIN_CANVAS_HOST_PORT` / config override)
- Browser profile CDP ports auto-allocate from `browser.controlPort + 9 .. + 108` (persisted per profile).

Checklist per instance:
- unique `gateway.port`
- unique `CLAWDBRAIN_CONFIG_PATH`
- unique `CLAWDBRAIN_STATE_DIR`
- unique `agents.defaults.workspace`
- separate WhatsApp numbers (if using WA)

Service install per profile:
```bash
clawdbrain --profile main gateway install
clawdbrain --profile rescue gateway install
```

Example:
```bash
CLAWDBRAIN_CONFIG_PATH=~/.clawdbrain/a.json CLAWDBRAIN_STATE_DIR=~/.clawdbrain-a clawdbrain gateway --port 19001
CLAWDBRAIN_CONFIG_PATH=~/.clawdbrain/b.json CLAWDBRAIN_STATE_DIR=~/.clawdbrain-b clawdbrain gateway --port 19002
```
...
## Methods (initial set)
- `health` — full health snapshot (same shape as `clawdbrain health --json`).
...
- Use launchd to keep the service alive:
  - Program: path to `clawdbrain`
  - Arguments: `gateway`
  - KeepAlive: true
  - StandardOut/Err: file paths or `syslog`
- On failure, launchd restarts; fatal misconfig should keep exiting so the operator notices.
- LaunchAgents are per-user and require a logged-in session; for headless setups use a custom LaunchDaemon (not shipped).
  - `clawdbrain gateway install` writes `~/Library/LaunchAgents/com.clawdbrain.gateway.plist`
    (or `com.clawdbrain.<profile>.plist`).
  - `clawdbrain doctor` audits the LaunchAgent config and can update it to current defaults.
...
- If other gateway-like services are detected, the CLI warns unless they are Clawdbrain profile services.
  ...
  - Cleanup: `clawdbrain gateway uninstall` (current service) and `clawdbrain doctor` (legacy migrations).
- `gateway install` is a no-op when already installed; use `clawdbrain gateway install --force` to reinstall (profile/env/path changes).

Bundled mac app:
- Clawdbrain.app can bundle a Node-based gateway relay and install a per-user LaunchAgent labeled
  `com.clawdbrain.gateway` (or `com.clawdbrain.<profile>`).
- To stop it cleanly, use `clawdbrain gateway stop` (or `launchctl bootout gui/$UID/com.clawdbrain.gateway`).
- To restart, use `clawdbrain gateway restart` (or `launchctl kickstart -k gui/$UID/com.clawdbrain.gateway`).
  - `launchctl` only works if the LaunchAgent is installed; otherwise use `clawdbrain gateway install` first.
  - Replace the label with `com.clawdbrain.<profile>` when running a named profile.

## Supervision (systemd user unit)
Clawdbrain installs a **systemd user service** by default on Linux/WSL2. We
...
`clawdbrain gateway install` writes the user unit. `clawdbrain doctor` audits the
unit and can update it to match the current recommended defaults.

Create `~/.config/systemd/user/clawdbrain-gateway[-<profile>].service`:
```
[Unit]
Description=Clawdbrain Gateway (profile: <profile>, v<version>)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/local/bin/clawdbrain gateway --port 18789
Restart=always
RestartSec=5
Environment=CLAWDBRAIN_GATEWAY_TOKEN=
...
```
systemctl --user enable --now clawdbrain-gateway[-<profile>].service
```

**Alternative (system service)** - for always-on or multi-user servers, you can
install a systemd **system** unit instead of a user unit (no lingering needed).
Create `/etc/systemd/system/clawdbrain-gateway[-<profile>].service` (copy the unit above,
switch `WantedBy=multi-user.target`, set `User=` + `WorkingDirectory=`), then:
```
sudo systemctl daemon-reload
sudo systemctl enable --now clawdbrain-gateway[-<profile>].service
```
...
## CLI helpers
- `clawdbrain gateway health|status` — request health/status over the Gateway WS.
- `clawdbrain message send --target <num> --message "hi" [--media ...]` — send via Gateway (idempotent for WhatsApp).
- `clawdbrain agent --message "hi" --to <num>` — run an agent turn (waits for final by default).
- `clawdbrain gateway call <method> --params '{"k":"v"}'` — raw method invoker for debugging.
- `clawdbrain gateway stop|restart` — stop/restart the supervised gateway service (launchd/systemd).
- Gateway helper subcommands assume a running gateway on `--url`; they no longer auto-spawn one.

## Migration guidance
- Retire uses of `clawdbrain gateway` and the legacy TCP control port.
