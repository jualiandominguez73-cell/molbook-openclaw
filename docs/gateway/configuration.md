---
summary: "All configuration options for ~/.clawdbrain/clawdbrain.json with examples"
read_when:
  - Adding or modifying config fields
---
# Configuration 🔧

Clawdbrain reads an optional **JSON5** config from `~/.clawdbrain/clawdbrain.json` (comments + trailing commas allowed).

If the file is missing, Clawdbrain uses safe-ish defaults (embedded Pi agent + per-sender sessions + workspace `~/clawd`). You usually only need a config to:
...
Clawdbrain only accepts configurations that fully match the schema.
Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start** for safety.

When validation fails:
- The Gateway does not boot.
- Only diagnostic commands are allowed (for example: `clawdbrain doctor`, `clawdbrain logs`, `clawdbrain health`, `clawdbrain status`, `clawdbrain service`, `clawdbrain help`).
- Run `clawdbrain doctor` to see the exact issues.
- Run `clawdbrain doctor --fix` (or `--yes`) to apply migrations/repairs.
...
use `config.patch` or `clawdbrain config set`. Keep a backup of `~/.clawdbrain/clawdbrain.json`.
...
```bash
clawdbrain gateway call config.get --params '{}' # capture payload.hash
clawdbrain gateway call config.apply --params '{
...
```bash
clawdbrain gateway call config.get --params '{}' # capture payload.hash
clawdbrain gateway call config.patch --params '{
...
{ "$include": "/etc/clawdbrain/base.json5" } // absolute
...
```json5
// ~/.clawdbrain/clients/mueller/agents.json5
[
...
```json5
// ~/.clawdbrain/clients/mueller/broadcast.json5
{
...
Clawdbrain reads env vars from the parent process (shell, launchd/systemd, CI, etc.).
...
Opt-in convenience: if enabled and none of the expected keys are set yet, Clawdbrain runs your login shell and imports only the missing expected keys (never overrides).
...
Env var equivalent:
- `CLAWDBRAIN_LOAD_SHELL_ENV=1`
- `CLAWDBRAIN_SHELL_ENV_TIMEOUT_MS=15000`
...
```json5
{
  ...
  gateway: {
    auth: {
      token: "${CLAWDBRAIN_GATEWAY_TOKEN}"
    }
  }
}
```
...
Clawdbrain stores **per-agent** auth profiles (OAuth + API keys) in:
- `<agentDir>/auth-profiles.json` (default: `~/.clawdbrain/agents/<agentId>/agent/auth-profiles.json`)

See also: [/concepts/oauth](/concepts/oauth)

Legacy OAuth imports:
- `~/.clawdbrain/credentials/oauth.json` (or `$CLAWDBRAIN_STATE_DIR/credentials/oauth.json`)

The embedded Pi agent maintains a runtime cache at:
- `<agentDir>/auth.json` (managed automatically; don’t edit manually)

Legacy agent dir (pre multi-agent):
- `~/.clawdbrain/agent/*` (migrated by `clawdbrain doctor` into `~/.clawdbrain/agents/<defaultAgentId>/agent/*`)

Overrides:
- OAuth dir (legacy import only): `CLAWDBRAIN_OAUTH_DIR`
- Agent dir (default agent root override): `CLAWDBRAIN_AGENT_DIR` (preferred), `PI_CODING_AGENT_DIR` (legacy)

On first use, Clawdbrain imports `oauth.json` entries into `auth-profiles.json`.
...
If set, Clawdbrain derives defaults (only when you haven’t set them explicitly):
...
- Default log file: `/tmp/clawdbrain/clawdbrain-YYYY-MM-DD.log`
- If you want a stable path, set `logging.file` to `/tmp/clawdbrain/clawdbrain.log`.
...
```json5
{
  logging: {
    level: "info",
    file: "/tmp/clawdbrain/clawdbrain.log",
    consoleLevel: "info",
    consoleStyle: "pretty",
  ...
  channels: {
    whatsapp: {
      accounts: {
        ...
        personal: {},
        biz: {
          // Optional override. Default: ~/.clawdbrain/credentials/whatsapp/biz
          // authDir: "~/.clawdbrain/credentials/whatsapp/biz",
        }
      }
    }
  }
...
- The legacy single-account Baileys auth dir is migrated by `clawdbrain doctor` into `whatsapp/default`.
...
```json5
{
  ...
  agents: {
    list: [
      { id: "main", groupChat: { mentionPatterns: ["@clawd", "clawdbrain", "clawd"] } }
    ]
  }
}
```
...
- `agents.list[]`: per-agent overrides.
  ...
  - `agentDir`: default `~/.clawdbrain/agents/<agentId>/agent`.
...
- `commands.config: true` enables `/config` (reads/writes `clawdbrain.json`).
...
Clawdbrain starts Telegram only when a `channels.telegram` config section exists. The bot token is resolved from `channels.telegram.botToken` (or `channels.telegram.tokenFile`), with `TELEGRAM_BOT_TOKEN` as a fallback for the default account.
...
Clawdbrain starts Slack when the provider is enabled and both tokens are set (via config or `SLACK_BOT_TOKEN` + `SLACK_APP_TOKEN`). Use `user:<id>` (DM) or `channel:<id>` when specifying delivery targets for cron/CLI commands.
...
Install it first: `clawdbrain plugins install @clawdbrain/mattermost` (or `./extensions/mattermost` from a git checkout).
...
Clawdbrain starts Mattermost when the account is configured (bot token + base URL) and enabled. The token + base URL are resolved from `channels.mattermost.botToken` + `channels.mattermost.baseUrl` or `MATTERMOST_BOT_TOKEN` + `MATTERMOST_URL` for the default account (unless `channels.mattermost.enabled` is `false`).
...
Clawdbrain spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
Optional repository root to show in the system prompt’s Runtime line. If unset, Clawdbrain
...
```json5
{
  agents: { defaults: { repoRoot: "~/Projects/clawdbrain" } }
}
```
...
When a file exceeds this limit, Clawdbrain logs a warning and injects a truncated
...
message envelopes). If unset, Clawdbrain uses the host timezone at runtime.
...
`[clawdbrain]`, so same-phone conversations stay legible.
...
`messages.messagePrefix`). Default stays **unchanged**: `"[clawdbrain]"` when
`channels.whatsapp.allowFrom` is empty, otherwise `""` (no prefix). When using
`"[clawdbrain]"`, Clawdbrain will instead use `[{identity.name}]` when the routed
...
Enable text-to-speech for outbound replies. When on, Clawdbrain generates audio
...
```json5
{
  messages: {
    tts: {
      ...
      maxTextLength: 4000,
      timeoutMs: 30000,
      prefsPath: "~/.clawdbrain/settings/tts.json",
      elevenlabs: {
...
Clawdbrain also ships a few built-in alias shorthands. Defaults only apply when the model
...
If you omit the provider, Clawdbrain currently assumes `anthropic` as a temporary
...
- `tools.web.search.apiKey` (recommended: set via `clawdbrain configure --section web`, or use `BRAVE_API_KEY` env var)
...
- `group:nodes`: `nodes`
- `group:clawdbrain`: all built-in Clawdbrain tools (excludes provider plugins)
...
- agent workspace access: `workspaceAccess: "none"` (default)
  - `"none"`: use a per-scope sandbox workspace under `~/.clawdbrain/sandboxes`
  ...
  agents: {
    defaults: {
      sandbox: {
        ...
        workspaceRoot: "~/.clawdbrain/sandboxes",
        docker: {
        ...
        browser: {
          enabled: false,
          image: "clawdbrain-sandbox-browser:bookworm-slim",
          containerPrefix: "clawdbrain-sbx-browser-",
          cdpPort: 9222,
          vncPort: 5900,
          noVncPort: 6080,
          headless: false,
...
Clawdbrain uses the **pi-coding-agent** model catalog. You can add custom providers
(LiteLLM, local OpenAI-compatible servers, Anthropic proxies, etc.) by writing
`~/.clawdbrain/agents/<agentId>/agent/models.json` or by defining the same schema inside your
Clawdbrain config under `models.providers`.
Provider-by-provider overview + examples: [/concepts/model-providers](/concepts/model-providers).

When `models.providers` is present, Clawdbrain writes/merges a `models.json` into
`~/.clawdbrain/agents/<agentId>/agent/` on startup:
...
OpenCode Zen is a multi-model gateway with per-model endpoints. Clawdbrain uses
...
- Shortcut: `clawdbrain onboard --auth-choice opencode-zen`.
...
Shortcut: `clawdbrain onboard --auth-choice zai-api-key`.
...
Notes:
- Set `MOONSHOT_API_KEY` in the environment or use `clawdbrain onboard --auth-choice moonshot-api-key`.
...
Notes:
- Set `KIMICODE_API_KEY` in the environment or use `clawdbrain onboard --auth-choice kimi-code-api-key`.
...
Notes:
- Set `SYNTHETIC_API_KEY` or use `clawdbrain onboard --auth-choice synthetic-api-key`.
...
Notes:
- Set `MINIMAX_API_KEY` environment variable or use `clawdbrain onboard --auth-choice minimax-api`.
...
- Override the agent config root with `CLAWDBRAIN_AGENT_DIR` (or `PI_CODING_AGENT_DIR`)
  if you want `models.json` stored elsewhere (default: `~/.clawdbrain/agents/main/agent`).
...
```json5
{
  session: {
    ...
    // Default is already per-agent under ~/.clawdbrain/agents/<agentId>/sessions/sessions.json
    // You can override with {agentId} templating:
    store: "~/.clawdbrain/agents/{agentId}/sessions/sessions.json",
...
- `resetByType`: per-session overrides for `dm`, `group`, and `thread`.
  - If you only set legacy `session.idleMinutes` without any `reset`/`resetByType`, Clawdbrain stays in idle-only mode for backward compatibility.
...
overrides. Applies to **bundled** skills and `~/.clawdbrain/skills` (workspace skills
...
from `~/.clawdbrain/extensions`, `<workspace>/.clawdbrain/extensions`, plus any
...
Clawdbrain can start a **dedicated, isolated** Chrome/Brave/Edge/Chromium instance for clawd and expose a small loopback control service.
...
- Note: the control server is started by the running gateway (Clawdbrain.app menubar, or `clawdbrain gateway`).
...
```json5
{
  gateway: {
    ...
    bind: "loopback",
    // controlUi: { enabled: true, basePath: "/clawdbrain" }
...
- Examples: `"/ui"`, `"/clawdbrain"`, `"/apps/clawdbrain"`.
...
- When a connection comes from one of these IPs, Clawdbrain uses `x-forwarded-for` (or `x-real-ip`) to determine the client IP for local pairing checks and HTTP auth/local checks.
- Only list proxies you fully control, and ensure they **overwrite** incoming `x-forwarded-for`.

Notes:
- `clawdbrain gateway` refuses to start unless `gateway.mode` is set to `local` (or you pass the override flag).
- `gateway.port` controls the single multiplexed port used for WebSocket + HTTP (control UI, hooks, A2UI).
- OpenAI Chat Completions endpoint: **disabled by default**; enable with `gateway.http.endpoints.chatCompletions.enabled: true`.
- Precedence: `--port` > `CLAWDBRAIN_GATEWAY_PORT` > `gateway.port` > default `18789`.
...
- `gateway.auth.password` can be set here, or via `CLAWDBRAIN_GATEWAY_PASSWORD` (recommended).
- `gateway.auth.allowTailscale` allows Tailscale Serve identity headers
  (`tailscale-user-login`) to satisfy auth when the request arrives on loopback
  with `x-forwarded-for`, `x-forwarded-proto`, and `x-forwarded-host`. Clawdbrain
...
macOS app behavior:
- Clawdbrain.app watches `~/.clawdbrain/clawdbrain.json` and switches modes live when `gateway.mode` or `gateway.remote.url` changes.
...
The Gateway watches `~/.clawdbrain/clawdbrain.json` (or `CLAWDBRAIN_CONFIG_PATH`) and applies changes automatically.
...
Files watched:
- `~/.clawdbrain/clawdbrain.json` (or `CLAWDBRAIN_CONFIG_PATH`)
...
- `CLAWDBRAIN_STATE_DIR` (sessions/creds)
- `agents.defaults.workspace` (memories)
- `gateway.port` (unique per instance)

Convenience flags (CLI):
- `clawdbrain --dev …` → uses `~/.clawdbrain-dev` + shifts ports from base `19001`
- `clawdbrain --profile <name> …` → uses `~/.clawdbrain-<name>` (port via config/env/flags)
...
Example:
```bash
CLAWDBRAIN_CONFIG_PATH=~/.clawdbrain/a.json \
CLAWDBRAIN_STATE_DIR=~/.clawdbrain-a \
clawdbrain gateway --port 19001
```
...
```json5
{
  hooks: {
    ...
    path: "/hooks",
    presets: ["gmail"],
    transformsDir: "~/.clawdbrain/hooks",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Gmail helper config (used by `clawdbrain webhooks gmail setup` / `run`):

```json5
{
  hooks: {
    gmail: {
      account: "clawdbrain@gmail.com",
...
- Set `CLAWDBRAIN_SKIP_GMAIL_WATCHER=1` to disable the auto-start (for manual runs).
...
Note: when `tailscale.mode` is on, Clawdbrain defaults `serve.path` to `/` so
...
- watches the directory and broadcasts reloads over a WebSocket endpoint at `/__clawdbrain/ws`
- auto-creates a starter `index.html` when the directory is empty (so you see something immediately)
- also serves A2UI at `/__clawdbrain__/a2ui/` and is advertised to nodes as `canvasHostUrl`
...
- env: `CLAWDBRAIN_SKIP_CANVAS_HOST=1`
...
```json5
{
  bridge: {
    ...
    tls: {
      enabled: true,
      // Uses ~/.clawdbrain/bridge/tls/bridge-{cert,key}.pem when omitted.
      // certPath: "~/.clawdbrain/bridge/tls/bridge-cert.pem",
      // keyPath: "~/.clawdbrain/bridge/tls/bridge-key.pem"
    }
  }
}
```
...
Controls LAN mDNS discovery broadcasts (`_clawdbrain-gw._tcp`).
...
When enabled, the Gateway writes a unicast DNS-SD zone for `_clawdbrain-bridge._tcp` under `~/.clawdbrain/dns/` using the standard discovery domain `clawdbrain.internal.`

To make iOS/Android discover across networks (Vienna ⇄ London), pair this with:
- a DNS server on the gateway host serving `clawdbrain.internal.` (CoreDNS is recommended)
- Tailscale **split DNS** so clients resolve `clawdbrain.internal` via that server
