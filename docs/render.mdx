...
Deploy Clawdbrain on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
<a href="https://render.com/deploy?repo=https://github.com/clawdbrain/clawdbrain" target="_blank" rel="noreferrer">Deploy to Render</a>
...
repository configures everything needed to run Clawdbrain:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: CLAWDBRAIN_STATE_DIR
        value: /data/.clawdbrain
      - key: CLAWDBRAIN_WORKSPACE_DIR
        value: /data/workspace
      - key: CLAWDBRAIN_GATEWAY_TOKEN
        generateValue: true  # auto-generates a secure token
    disk:
      name: clawdbrain-data
      mountPath: /data
      sizeGB: 1
```
...
The web dashboard is available at `https://<your-service>.onrender.com/clawdbrain`.
...
If you use the original Clawdbrain repository, Render will not auto-deploy your Clawdbrain. To update it, run a manual Blueprint sync from the dashboard.
...
For Clawdbrain, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Clawdbrain host.
