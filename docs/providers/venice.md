---
summary: "Use Venice AI privacy-focused models in Clawdbrain"
read_when:
  - You want privacy-focused inference in Clawdbrain
...
```bash
clawdbrain onboard --auth-choice venice-api-key
```
...
```bash
clawdbrain chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Clawdbrain shows all available Venice models. Pick based on your needs:
...
```bash
clawdbrain models set venice/claude-opus-45
clawdbrain models set venice/llama-3.3-70b
```

List all available models:

```bash
clawdbrain models list | grep venice
```

## Configure via `clawdbrain configure`

1. Run `clawdbrain configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Clawdbrain automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
clawdbrain chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
clawdbrain chat --model venice/claude-opus-45

# Use uncensored model
clawdbrain chat --model venice/venice-uncensored

# Use vision model with image
clawdbrain chat --model venice/qwen3-vl-235b-a22b

# Use coding model
clawdbrain chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `clawdbrain models list` to see currently available models. Some models may be temporarily offline.
