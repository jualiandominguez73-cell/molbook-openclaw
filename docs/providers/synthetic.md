---
summary: "Use Synthetic's Anthropic-compatible API in Clawdbrain"
read_when:
...
Synthetic exposes Anthropic-compatible endpoints. Clawdbrain registers it as the
...
```bash
clawdbrain onboard --auth-choice synthetic-api-key
```
...
Note: Clawdbrain's Anthropic client appends `/v1` to the base URL, so use
