...
```bash
clawdbrain plugins install @clawdbrain/msteams
```

Local checkout (when running from a git repo):
```bash
clawdbrain plugins install ./extensions/msteams
```

If you choose Teams during configure/onboarding and a git checkout is detected,
Clawdbrain will offer the local install path automatically.
...
3) Configure Clawdbrain with those credentials.
...
## Goals
- Talk to Clawdbrain via Teams DMs, group chats, or channels.
...
- On startup, Clawdbrain resolves team/channel and user allowlist names to IDs (when Graph permissions allow)
...
5. Configure `msteams` in `~/.clawdbrain/clawdbrain.json` (or env vars) and start the gateway.
6. The gateway listens for Bot Framework webhook traffic on `/api/messages` by default.

## Azure Bot Setup (Prerequisites)

Before configuring Clawdbrain, you need to create an Azure Bot resource.
...
2. Fill in the **Basics** tab:
   ...
   | Field | Value |
   |-------|-------|
   | **Bot handle** | Your bot name, e.g., `clawdbrain-msteams` (must be unique) |
...
1. **Install the Microsoft Teams plugin**
   - From npm: `clawdbrain plugins install @clawdbrain/msteams`
   - From a local checkout: `clawdbrain plugins install ./extensions/msteams`
...
```json
{
  ...
  "description": { "short": "Clawdbrain in Teams", "full": "Clawdbrain in Teams" },
...
Clawdbrain handles this by returning quickly and sending replies proactively, but very slow responses may still cause issues.
...
By default, Clawdbrain only downloads media from Microsoft/Teams hostnames. Override with `channels.msteams.mediaAllowHosts` (use `["*"]` to allow any host).
...
Uploaded files are stored in a `/ClawdbrainShared/` folder in the configured SharePoint site's default document library.

## Polls (Adaptive Cards)
Clawdbrain sends Teams polls as Adaptive Cards (there is no native Teams poll API).

- CLI: `clawdbrain message poll --channel msteams --target conversation:<id> ...`
- Votes are recorded by the gateway in `~/.clawdbrain/msteams-polls.json`.
...
**CLI:**
```bash
clawdbrain message send --channel msteams \
...
clawdbrain message send --channel msteams --target "user:40a1a0ed-..." --message "Hello"

# Send to a user by display name (triggers Graph API lookup)
clawdbrain message send --channel msteams --target "user:John Smith" --message "Hello"

# Send to a group chat or channel
clawdbrain message send --channel msteams --target "conversation:19:abc...@thread.tacv2" --message "Hello"

# Send an Adaptive Card to a conversation
clawdbrain message send --channel msteams --target "conversation:19:abc...@thread.tacv2" \
  --card '{"type":"AdaptiveCard","version":"1.5","body":[{"type":"TextBlock","text":"Hello"}]}'
```
