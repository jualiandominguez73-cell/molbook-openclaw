...
Tlon is a decentralized messenger built on Urbit. Clawdbrain connects to your Urbit ship and can
...
```bash
clawdbrain plugins install @clawdbrain/tlon
```

Local checkout (when running from a git repo):

```bash
clawdbrain plugins install ./extensions/tlon
```
...
Use these with `clawdbrain message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Clawdbrain replies in-thread.
