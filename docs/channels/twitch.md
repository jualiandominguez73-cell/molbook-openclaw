...
read_when:
  - Setting up Twitch chat integration for Clawdbrain
---
# Twitch (plugin)

Twitch chat support via IRC connection. Clawdbrain connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
clawdbrain plugins install @clawdbrain/twitch
```

Local checkout (when running from a git repo):

```bash
clawdbrain plugins install ./extensions/twitch
```
...
4) Configure the token:
   - Env: `CLAWDBRAIN_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "clawdbrain",              // Bot's Twitch account
      accessToken: "oauth:abc123...",    // OAuth Access Token (or use CLAWDBRAIN_TWITCH_ACCESS_TOKEN env var)
...
```bash
CLAWDBRAIN_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
