...
```bash
clawdbrain plugins install @clawdbrain/nextcloud-talk
```

Local checkout (when running from a git repo):
```bash
clawdbrain plugins install ./extensions/nextcloud-talk
```
...
2) On your Nextcloud server, create a bot:
   ```bash
   ./occ talk:bot:install "Clawdbrain" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `clawdbrain pairing list nextcloud-talk`
  - `clawdbrain pairing approve nextcloud-talk <CODE>`
