...
Matrix is an open, decentralized messaging protocol. Clawdbrain connects as a Matrix **user**
...
```bash
clawdbrain plugins install @clawdbrain/matrix
```

Local checkout (when running from a git repo):

```bash
clawdbrain plugins install ./extensions/matrix
```
...
1) Install the Matrix plugin:
   - From npm: `clawdbrain plugins install @clawdbrain/matrix`
   - From a local checkout: `clawdbrain plugins install ./extensions/matrix`
...
3) Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Clawdbrain calls the same
     login endpoint, stores the access token in `~/.clawdbrain/credentials/matrix/credentials.json`,
...
- On first connection, Clawdbrain requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Clawdbrain logs a warning.
...
`~/.clawdbrain/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `clawdbrain pairing list matrix`
  - `clawdbrain pairing approve matrix <CODE>`
...
- On startup, Clawdbrain resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are kept as typed.
