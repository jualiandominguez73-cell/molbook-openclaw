...
read_when:
  - "You installed Clawdbrain but `clawdbrain` is “command not found”"
...
Clawdbrain’s runtime baseline is **Node 22+**.

If you can run `npm install -g clawdbrain@latest` but later see `clawdbrain: command not found`, it’s almost always a **PATH** issue: the directory where npm puts global binaries isn’t on your shell’s PATH.
