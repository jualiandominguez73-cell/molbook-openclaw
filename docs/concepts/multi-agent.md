...
- **Session store** (chat history + routing state) under `~/.clawdbrain/agents/<agentId>/sessions`.
...
available from `~/.clawdbrain/skills`. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills).
...
- State dir: `~/.clawdbrain` (or `CLAWDBRAIN_STATE_DIR`)
- Workspace: `~/clawd` (or `~/clawd-<agentId>`)
- Agent dir: `~/.clawdbrain/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.clawdbrain/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Clawdbrain runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/clawd` (or `~/clawd-<profile>` when `CLAWDBRAIN_PROFILE` is set).
- State defaults to `~/.clawdbrain/agents/main/agent`.
...
```bash
clawdbrain agents list --bindings
```
...
`~/.clawdbrain/clawdbrain.json` (JSON5):
...
```js
{
    ...
    list: [
      {
        ...
        agentDir: "~/.clawdbrain/agents/home/agent",
      },
      {
        id: "work",
        name: "Work",
        workspace: "~/clawd-work",
        agentDir: "~/.clawdbrain/agents/work/agent",
      },
    ],
  },
  ...
  channels: {
    whatsapp: {
      accounts: {
        personal: {
          // Optional override. Default: ~/.clawdbrain/credentials/whatsapp/personal
          // authDir: "~/.clawdbrain/credentials/whatsapp/personal",
        },
        biz: {
