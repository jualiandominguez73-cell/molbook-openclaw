...
- `/usage cost` in chats: local cost summary aggregated from Clawdbrain session logs.
- CLI: `clawdbrain status --usage` prints a full per-provider breakdown.
- CLI: `clawdbrain channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
