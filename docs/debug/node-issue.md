...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Clawdbrain
---

# Node + tsx "__name is not a function" crash

## Summary
Running Clawdbrain via Node with `tsx` fails at startup with:

```
[clawdbrain] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `clawdbrain status` and `gateway:watch` worked.
