...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `clawdbrain sandbox explain`.
...
Legacy `agent.*` configs are migrated by `clawdbrain doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ```exec
   docker ps --filter "label=clawdbrain.sandbox=1"
   ```
...
4. **Monitor logs:**
   ```exec
   tail -f "${CLAWDBRAIN_STATE_DIR:-$HOME/.clawdbrain}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
