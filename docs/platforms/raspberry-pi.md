---
summary: "Clawdbrain on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Clawdbrain on a Raspberry Pi
  - Running Clawdbrain on ARM devices
  - Building a cheap always-on personal AI
---

# Clawdbrain on Raspberry Pi

## Goal

Run a persistent, always-on Clawdbrain Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
# Update config
clawdbrain config set gateway.bind tailnet
...
Most Clawdbrain features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled clawdbrain

# Enable if not
sudo systemctl enable clawdbrain

# Start on boot
sudo systemctl start clawdbrain
```
...
```bash
# Check logs
journalctl -u clawdbrain --no-pager -n 100

# Common fix: rebuild
cd ~/clawdbrain  # if using hackable install
npm run build
sudo systemctl restart clawdbrain
```
