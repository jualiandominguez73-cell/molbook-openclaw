---
summary: "Run Clawdbrain Gateway on exe.dev (VM + HTTPS proxy) for remote access"
read_when:
...
Goal: Clawdbrain Gateway running on an exe.dev VM, reachable from your laptop via:
...
1) Create VM → install Node 22 → install Clawdbrain  
2) Run `clawdbrain onboard --install-daemon`  
...
```bash
ssh exe.dev new --name=clawdbrain
```

Then connect:

```bash
ssh clawdbrain.exe.xyz
```

Tip: keep this VM **stateful**. Clawdbrain stores state under `~/.clawdbrain/` and `~/clawd/`.
...
```bash
ssh -N -L 18789:127.0.0.1:18789 clawdbrain.exe.xyz
```
...
```bash
export CLAWDBRAIN_GATEWAY_TOKEN="$(openssl rand -hex 32)"
clawdbrain gateway --bind lan --port 8080 --token "$CLAWDBRAIN_GATEWAY_TOKEN"
```

For service runs, persist it in `~/.clawdbrain/clawdbrain.json`:
...
Notes:
- Non-loopback binds require `gateway.auth.token` (or `CLAWDBRAIN_GATEWAY_TOKEN`).
- `gateway.remote.token` is only for remote CLI calls; it does not enable local auth.

Then point exe.dev’s proxy at `8080` (or whatever port you chose) and open your VM’s HTTPS URL:

```bash
ssh exe.dev share port clawdbrain 8080
```

Open:
- `https://clawdbrain.exe.xyz/`
...
On Linux, Clawdbrain uses a systemd **user** service. After `--install-daemon`, verify:

```bash
systemctl --user status clawdbrain-gateway[-<profile>].service
```
