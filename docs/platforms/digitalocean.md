---
summary: "Clawdbrain on DigitalOcean (simple paid VPS option)"
read_when:
  - Setting up Clawdbrain on DigitalOcean
  - Looking for cheap VPS hosting for Clawdbrain
---

# Clawdbrain on DigitalOcean

## Goal

Run a persistent Clawdbrain Gateway on DigitalOcean for **$6/month** (or $4/mo with reserved pricing).
...
# Check service
systemctl --user status clawdbrain-gateway.service

# View logs
journalctl --user -u clawdbrain-gateway.service -f
```
...
### WhatsApp
```bash
clawdbrain channels login whatsapp
# Scan QR code
```
...
journalctl -u clawdbrain --no-pager -n 50
```
