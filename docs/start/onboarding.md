---
summary: "First-run onboarding flow for Clawdbrain (macOS app)"
read_when:
...
- Writes credentials to `~/.clawdbrain/credentials/oauth.json`
...
The app can install the global `clawdbrain` CLI via npm/pnpm so terminal
...
On the first agent run, Clawdbrain bootstraps a workspace (default `~/clawd`):
...
```bash
clawdbrain webhooks gmail setup --account you@gmail.com
```
...
- `~/.clawdbrain/credentials/oauth.json`
