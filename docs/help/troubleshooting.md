...
You can also set `CLAWDBRAIN_VERBOSE=1` instead of the flag.
...
### `docs.clawdbrain.bot` shows an SSL error (Comcast/Xfinity)

Some Comcast/Xfinity connections block `docs.clawdbrain.bot` via Xfinity Advanced Security.
Disable Advanced Security or add `docs.clawdbrain.bot` to the allowlist, then retry.
...
- Check the allowlist: `clawdbrain config get agents.defaults.models`
...
If you can, include the relevant log tail from `clawdbrain logs --follow`.
