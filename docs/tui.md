...
Remote Gateway:
```bash
clawdbrain tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `clawdbrain tui --deliver`
...
- Check the Gateway logs: `clawdbrain logs --follow`.
- Confirm the agent can run: `clawdbrain status` and `clawdbrain models status`.
- If you expect messages in a chat channel, enable delivery (`/deliver on` or `--deliver`).
- `--history-limit <n>`: History entries to load (default 200)

## Troubleshooting
- `disconnected`: ensure the Gateway is running and your `--url/--token/--password` are correct.
- No agents in picker: check `clawdbrain agents list` and your routing config.
