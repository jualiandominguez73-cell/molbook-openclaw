---
title: Lobster
summary: "Typed workflow runtime for Clawdbrain with resumable approval gates."
description: Typed workflow runtime for Clawdbrain — composable pipelines with approval gates.
read_when:
...
Lobster is a workflow shell that lets Clawdbrain run multi-step tool sequences as a single, deterministic operation with explicit approval checkpoints.
...
- **One call instead of many**: Clawdbrain runs one Lobster tool call and gets a structured result.
...
Clawdbrain launches the local `lobster` CLI in **tool mode** and parses a JSON envelope from stdout.
...
Lobster can run YAML/JSON workflow files with `name`, `args`, `steps`, `env`, `condition`, and `approval` fields. In Clawdbrain tool calls, set `pipeline` to the file path.
...
Install the Lobster CLI on the **same host** that runs the Clawdbrain Gateway (see the [Lobster repo](https://github.com/clawdbrain/lobster)), and ensure `lobster` is on `PATH`.
...
plugin tools (like `lobster`), Clawdbrain keeps core tools enabled. To restrict core
