---
summary: "Chrome extension: let Clawdbrain drive your existing Chrome tab"
read_when:
...
The Clawdbrain Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate clawd-managed Chrome profile.
...
Clawdbrain then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Clawdbrain release (npm package) as static files. There is no separate “build” step.

After upgrading Clawdbrain:
- Re-run `clawdbrain browser extension install` to refresh the installed files under your Clawdbrain state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Clawdbrain ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Clawdbrain to control.
...
- `ON`: attached; Clawdbrain can drive that tab.
...
Debugging: `clawdbrain sandbox explain`
...
`clawdbrain browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `clawdbrain browser extension install` first to copy the extension to a stable location under your Clawdbrain state directory.
