...
```json5
{
  browser: {
    ...
    actTimeoutMs: 30000, // browser act/interaction HTTP timeout (ms)
...
- `actTimeoutMs` controls the HTTP timeout for browser actions (click, type, navigate, snapshot, etc.). Default: `30000` (30s). Increase to `60000` or more if actions time out on slow-loading pages.
