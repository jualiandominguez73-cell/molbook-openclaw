...
- **gateway host** → `clawdbrain` process on the gateway machine
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.clawdbrain/exec-approvals.sock",
...
`~/.clawdbrain/exec-approvals.json` directly.

CLI: `clawdbrain approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
