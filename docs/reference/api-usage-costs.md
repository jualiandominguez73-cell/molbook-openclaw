...
Clawdbrain features that can generate provider usage or paid API calls.
...
- `clawdbrain status --usage` and `clawdbrain channels list` show provider **usage windows**
...
Clawdbrain can pick up credentials from:
...
- `clawdbrain status --usage`
- `clawdbrain models status --json`
...
### 8) Model scan / probe
`clawdbrain models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
