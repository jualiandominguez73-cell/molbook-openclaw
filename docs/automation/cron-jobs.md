...
- Jobs persist under `~/.clawdbrain/cron/` so restarts don’t lose schedules.
...
## Storage & history
- Job store: `~/.clawdbrain/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.clawdbrain/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.clawdbrain/cron/jobs.json",
...
clawdbrain cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
clawdbrain cron edit <jobId> --agent ops
clawdbrain cron edit <jobId> --clear-agent
```
```
...
```bash
clawdbrain system event --mode now --text "Next heartbeat: check battery."
```

## Gateway API surface
- `cron.list`, `cron.status`, `cron.add`, `cron.update`, `cron.remove`
- `cron.run` (force or due), `cron.runs`
For immediate system events without a job, use [`clawdbrain system event`](/cli/system).

## Troubleshooting

### “Nothing runs”
- Check cron is enabled: `cron.enabled` and `CLAWDBRAIN_SKIP_CRON`.
