...
Delivery:
- Publish as `@clawdbrain/plugin-sdk` (or export from core under `clawdbrain/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)
Scope: everything that touches core runtime behavior.
Accessed via `ClawdbrainPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: ClawdbrainConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: ClawdbrainConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: ClawdbrainConfig, agentId?: string): RegExp[];
      matchesMentionPatterns(text: string, regexes: RegExp[]): boolean;
    };
    groups: {
      resolveGroupPolicy(cfg: ClawdbrainConfig, channel: string, accountId: string, groupId: string): {
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: ClawdbrainConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: ClawdbrainConfig): string;
  };
};
```
...
### Phase 0: scaffolding
- Introduce `@clawdbrain/plugin-sdk`.
- Add `api.runtime` to `ClawdbrainPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `clawdbrainRuntime: ">=2026.2.0"`).
