...
- Plugin manifests (`clawdbrain.plugin.json`) are required for all plugins.

## Plugin schema enforcement
- Each plugin provides a strict JSON Schema for its config (inline in the manifest).
- Plugin load flow:
  1) Resolve plugin manifest + schema (`clawdbrain.plugin.json`).
...
- If config invalid:
  ...
  - Instruct: `clawdbrain doctor --fix`.
...
Everything else must hard-fail with: “Config invalid. Run `clawdbrain doctor --fix`.”
