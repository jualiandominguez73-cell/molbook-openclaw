# Clawdbrain ACP Bridge

This document describes how the Clawdbrain ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`clawdbrain acp` exposes an ACP agent over stdio and forwards prompts to a running
Clawdbrain Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Clawdbrain Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `clawdbrain acp` over stdio.

Example config:

```bash
clawdbrain config set gateway.remote.url wss://gateway-host:18789
clawdbrain config set gateway.remote.token <token>
```

Example run:

```bash
clawdbrain acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
clawdbrain acp --session agent:main:main
clawdbrain acp --session agent:design:main
clawdbrain acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Clawdbrain ACP” to start a thread.

## Execution Model

- ACP client spawns `clawdbrain acp` and speaks ACP messages over stdio.
...
```bash
clawdbrain acp --session agent:main:main
clawdbrain acp --session-label "support inbox"
clawdbrain acp --reset-session
```
...
`clawdbrain acp` resolves the Gateway URL and auth from CLI flags or config:
