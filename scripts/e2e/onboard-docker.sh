...
IMAGE_NAME="clawdbrain-onboard-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  cat > /tmp/clawdbrain-bin/trash <<'"'"'TRASH'"'"'
#!/usr/bin/env bash
set -euo pipefail
...
TRASH
  chmod +x /tmp/clawdbrain-bin/trash
  ...
  run_wizard_cmd() {
    ...
    input_fifo="$(mktemp -u "/tmp/clawdbrain-onboard-${case_name}.XXXXXX")"
    mkfifo "$input_fifo"
    local log_path="/tmp/clawdbrain-onboard-${case_name}.log"
  ...
  make_home() {
    mktemp -d "/tmp/clawdbrain-e2e-$1.XXXXXX"
  }
  ...
  run_case_local_basic() {
    ...
    config_path="$HOME/.clawdbrain/clawdbrain.json"
    sessions_dir="$HOME/.clawdbrain/agents/main/sessions"
...
NODE
  ...
  run_case_reset() {
    ...
    cat > "$HOME/.clawdbrain/clawdbrain.json" <<'"'"'JSON'"'"'
{
