...
IMAGE_NAME="clawdbrain-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="clawdbrain-net-e2e-$$"
GW_NAME="clawdbrain-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "CLAWDBRAIN_GATEWAY_TOKEN=$TOKEN" \
	  -e "CLAWDBRAIN_SKIP_CHANNELS=1" \
	  -e "CLAWDBRAIN_SKIP_GMAIL_WATCHER=1" \
	  -e "CLAWDBRAIN_SKIP_CRON=1" \
	  -e "CLAWDBRAIN_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
