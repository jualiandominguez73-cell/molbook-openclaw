...
IMAGE_NAME="clawdbrain-doctor-install-switch-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  export PATH="/tmp/clawdbrain-bin:$PATH"
  mkdir -p /tmp/clawdbrain-bin

  cat > /tmp/clawdbrain-bin/systemctl <<"SYSTEMCTL"
#!/usr/bin/env bash
set -euo pipefail
...
SYSTEMCTL
  chmod +x /tmp/clawdbrain-bin/systemctl

  cat > /tmp/clawdbrain-bin/loginctl <<"LOGINCTL"
#!/usr/bin/env bash
set -euo pipefail
...
exit 0
LOGINCTL
  chmod +x /tmp/clawdbrain-bin/loginctl
  ...
  npm_bin="/tmp/npm-prefix/bin/clawdbrain"
  npm_entry="/tmp/npm-prefix/lib/node_modules/clawdbrain/dist/entry.js"
  ...
  run_flow() {
    ...
    echo "== Flow: $name =="
    home_dir=$(mktemp -d "/tmp/clawdbrain-switch-${name}.XXXXXX")
    export HOME="$home_dir"
    export USER="testuser"

    eval "$install_cmd"

    unit_path="$HOME/.config/systemd/user/clawdbrain-gateway.service"
    if [ ! -f "$unit_path" ]; then
