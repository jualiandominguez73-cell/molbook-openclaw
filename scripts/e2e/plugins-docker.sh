...
IMAGE_NAME="clawdbrain-plugins-e2e"
...
docker run --rm -t "$IMAGE_NAME" bash -lc '
  ...
  home_dir=$(mktemp -d "/tmp/clawdbrain-plugins-e2e.XXXXXX")
  export HOME="$home_dir"
  mkdir -p "$HOME/.clawdbrain/extensions"

  cat > "$HOME/.clawdbrain/extensions/demo-plugin.js" <<'"'"'JS'"'"'
module.exports = {
  id: "demo-plugin",
  name: "Demo Plugin",
...
NODE
  ...
  pack_dir="$(mktemp -d "/tmp/clawdbrain-plugin-pack.XXXXXX")"
  mkdir -p "$pack_dir/package"
  cat > "$pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@clawdbrain/demo-plugin-tgz",
  "version": "0.0.1",
...
NODE
  ...
  dir_plugin="$(mktemp -d "/tmp/clawdbrain-plugin-dir.XXXXXX")"
...
NODE
  ...
  file_pack_dir="$(mktemp -d "/tmp/clawdbrain-plugin-filepack.XXXXXX")"
  mkdir -p "$file_pack_dir/package"
  cat > "$file_pack_dir/package/package.json" <<'"'"'JSON'"'"'
{
  "name": "@clawdbrain/demo-plugin-file",
  "version": "0.0.1",
  "clawdbrain": { "extensions": ["./index.js"] }
}
JSON
