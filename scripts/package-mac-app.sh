...
# Build and bundle Clawdbrain into a minimal .app we can open.
# Outputs to dist/Clawdbrain.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Clawdbrain.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Clawdbrain"
BUNDLE_ID="${BUNDLE_ID:-com.clawdbrain.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/clawdbrain/clawdbrain/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Clawdbrain/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :ClawdbrainBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :ClawdbrainGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Clawdbrain"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Clawdbrain"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Clawdbrain"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Clawdbrain" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Clawdbrain/Resources/Clawdbrain.icns" "$APP_ROOT/Contents/Resources/Clawdbrain.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Clawdbrain/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying ClawdbrainKit resources"
CLAWDBRAINKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/ClawdbrainKit_ClawdbrainKit.bundle"
if [ -d "$CLAWDBRAINKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/ClawdbrainKit_ClawdbrainKit.bundle"
  cp -R "$CLAWDBRAINKIT_BUNDLE" "$APP_ROOT/Contents/Resources/ClawdbrainKit_ClawdbrainKit.bundle"
else
  echo "WARN: ClawdbrainKit resource bundle not found at $CLAWDBRAINKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Clawdbrain"
killall -q Clawdbrain 2>/dev/null || true
