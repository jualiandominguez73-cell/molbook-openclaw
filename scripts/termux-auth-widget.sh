...
# Clawdbrain Auth Widget for Termux
...
SERVER="${CLAWDBRAIN_SERVER:-l36}"

# Check auth status
termux-toast "Checking Clawdbrain auth..."

STATUS=$(ssh "$SERVER" '$HOME/clawdbrain/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/clawdbrain/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|CLAWDBRAIN_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/clawdbrain/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|CLAWDBRAIN_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Clawdbrain won't work until re-auth"
                ;;
        esac
        ;;
