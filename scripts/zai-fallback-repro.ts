...
async function main() {
  ...
  const baseDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "clawdbrain-zai-fallback-"),
  );
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "clawdbrain.json");
  ...
  const sessionId =
    process.env.CLAWDBRAIN_ZAI_FALLBACK_SESSION_ID ?? randomUUID();
