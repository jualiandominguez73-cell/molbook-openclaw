...
SMOKE_PREVIOUS_VERSION="${CLAWDBRAIN_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${CLAWDBRAIN_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="clawdbrain"
if [[ -z "${CLAWDBRAIN_INSTALL_PACKAGE:-}" && "$INSTALL_URL" == *"clawdbrain.bot"* ]]; then
  DEFAULT_PACKAGE="clawdbrain"
fi
PACKAGE_NAME="${CLAWDBRAIN_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
if [[ "$PACKAGE_NAME" == "clawdbrain" ]]; then
  ALT_PACKAGE_NAME="clawdbrain"
else
  ALT_PACKAGE_NAME="clawdbrain"
fi
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (CLAWDBRAIN_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${CLAWDBRAIN_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "$CLAWDBRAIN_INSTALL_LATEST_OUT"
fi
