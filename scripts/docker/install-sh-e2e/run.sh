...
INSTALL_URL="${CLAWDBRAIN_INSTALL_URL:-https://clawdbrain.bot/install.sh}"
MODELS_MODE="${CLAWDBRAIN_E2E_MODELS:-both}" # both|openai|anthropic
INSTALL_TAG="${CLAWDBRAIN_INSTALL_TAG:-latest}"
E2E_PREVIOUS_VERSION="${CLAWDBRAIN_INSTALL_E2E_PREVIOUS:-}"
SKIP_PREVIOUS="${CLAWDBRAIN_INSTALL_E2E_SKIP_PREVIOUS:-0}"
OPENAI_API_KEY="${OPENAI_API_KEY:-}"
ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
ANTHROPIC_API_TOKEN="${ANTHROPIC_API_TOKEN:-}"

if [[ "$MODELS_MODE" != "both" && "$MODELS_MODE" != "openai" && "$MODELS_MODE" != "anthropic" ]]; then
  echo "ERROR: CLAWDBRAIN_E2E_MODELS must be one of: both|openai|anthropic" >&2
  exit 2
fi

if [[ "$MODELS_MODE" == "both" ]]; then
  if [[ -z "$OPENAI_API_KEY" ]]; then
    echo "ERROR: CLAWDBRAIN_E2E_MODELS=both requires OPENAI_API_KEY." >&2
    exit 2
  fi
  if [[ -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
    echo "ERROR: CLAWDBRAIN_E2E_MODELS=both requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
    exit 2
  fi
elif [[ "$MODELS_MODE" == "openai" && -z "$OPENAI_API_KEY" ]]; then
  echo "ERROR: CLAWDBRAIN_E2E_MODELS=openai requires OPENAI_API_KEY." >&2
  exit 2
elif [[ "$MODELS_MODE" == "anthropic" && -z "$ANTHROPIC_API_TOKEN" && -z "$ANTHROPIC_API_KEY" ]]; then
  echo "ERROR: CLAWDBRAIN_E2E_MODELS=anthropic requires ANTHROPIC_API_TOKEN or ANTHROPIC_API_KEY." >&2
  exit 2
fi

echo "==> Resolve npm versions"
EXPECTED_VERSION="$(npm view "clawdbrain@${INSTALL_TAG}" version)"
if [[ -z "$EXPECTED_VERSION" || "$EXPECTED_VERSION" == "undefined" || "$EXPECTED_VERSION" == "null" ]]; then
  echo "ERROR: unable to resolve clawdbrain@${INSTALL_TAG} version" >&2
  exit 2
fi
...
const versions = JSON.parse(execSync("npm view clawdbrain versions --json", { encoding: "utf8" }));
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (CLAWDBRAIN_INSTALL_E2E_SKIP_PREVIOUS=1)"
else
  echo "==> Preinstall previous (forces installer upgrade path; avoids read() prompt)"
  npm install -g "clawdbrain@${PREVIOUS_VERSION}"
fi

echo "==> Run official installer one-liner"
if [[ "$INSTALL_TAG" == "beta" ]]; then
  CLAWDBRAIN_BETA=1 curl -fsSL "$INSTALL_URL" | bash
elif [[ "$INSTALL_TAG" != "latest" ]]; then
  CLAWDBRAIN_VERSION="$INSTALL_TAG" curl -fsSL "$INSTALL_URL" | bash
else
  curl -fsSL "$INSTALL_URL" | bash
fi

echo "==> Verify installed version"
INSTALLED_VERSION="$(clawdbrain --version 2>/dev/null | head -n 1 | tr -d '\r')"
echo "installed=$INSTALLED_VERSION expected=$EXPECTED_VERSION"
if [[ "$INSTALLED_VERSION" != "$EXPECTED_VERSION" ]]; then
  echo "ERROR: expected clawdbrain@$EXPECTED_VERSION, got clawdbrain@$INSTALLED_VERSION" >&2
  exit 1
fi
...
set_image_model() {
  ...
  local candidate
  for candidate in "$@"; do
    if clawdbrain --profile "$profile" models set-image "$candidate" >/dev/null 2>&1; then
      echo "$candidate"
      return 0
    fi
  done
...
set_agent_model() {
  ...
  shift
  for candidate in "$@"; do
    if clawdbrain --profile "$profile" models set "$candidate" >/dev/null 2>&1; then
      echo "$candidate"
      return 0
    fi
  done
...
run_agent_turn() {
  ...
  local prompt="$3"
  local out_json="$4"
  clawdbrain --profile "$profile" agent \
...
run_profile() {
  ...
  local agent_model_provider="$4" # "openai"|"anthropic"
	  ...
	  if [[ "$agent_model_provider" == "openai" ]]; then
	    clawdbrain --profile "$profile" onboard \
	      --non-interactive \
	      --accept-risk \
	      --flow quickstart \
  ...
  SESSION_JSONL="/root/.clawdbrain-${profile}/agents/main/sessions/${SESSION_ID}.jsonl"
  ...
  clawdbrain --profile "$profile" gateway --port "$port" --bind loopback >"$GATEWAY_LOG" 2>&1 &
  GATEWAY_PID="$!"
  cleanup_profile() {
  ...
  for _ in $(seq 1 60); do
    if clawdbrain --profile "$profile" health --timeout 2000 --json >/dev/null 2>&1; then
      break
    fi
    sleep 0.25
  done
  clawdbrain --profile "$profile" health --timeout 10000 --json >/dev/null
  ...
  if [[ ! -f "$SESSION_JSONL" ]]; then
    ...
    ls -la "/root/.clawdbrain-${profile}/agents/main/sessions" >&2 || true
    exit 1
  fi
