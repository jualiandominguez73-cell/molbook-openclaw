...
BASE_IMAGE="${BASE_IMAGE:-clawdbrain-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-clawdbrain-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=clawdbrain.sandbox=1)
NOTE
