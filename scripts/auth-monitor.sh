...
#
# Suggested cron: */30 * * * * /home/admin/clawdbrain/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Clawdbrain notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., clawdbrain-alerts)
...
STATE_FILE="$HOME/.clawdbrain/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Clawdbrain if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use clawdbrain
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Clawdbrain to $NOTIFY_PHONE..."
            clawdbrain send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Clawdbrain is down. Run: ssh l36 '~/clawdbrain/scripts/mobile-reauth.sh'" "urgent"
    exit 1
