...
ZIP=${1:?"Usage: $0 Clawdbrain-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/clawdbrain/clawdbrain/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Clawdbrain-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/clawdbrain/clawdbrain/releases/download/v${VERSION}/"}
