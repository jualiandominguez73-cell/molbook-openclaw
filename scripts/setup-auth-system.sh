#!/bin/bash
# Setup Clawdbrain Auth Management System
...
echo "=== Clawdbrain Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'clawdbrain-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Clawdbrain message: Send warning via Clawdbrain itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/clawdbrain-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/clawdbrain-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/clawdbrain-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now clawdbrain-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status clawdbrain-auth-monitor.timer"
echo ""
echo "Quick commands:"
