...
import type { ClawdbrainConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("clawdbrain-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("clawdbrain-bin-sherpa-");
      const modelDir = await makeTempDir("clawdbrain-sherpa-model-");
      ...
      const cfg: ClawdbrainConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("clawdbrain-bin-whispercpp-");
      const modelDir = await makeTempDir("clawdbrain-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("clawdbrain-bin-gemini-");
      ...
      const cfg: ClawdbrainConfig = { tools: { media: { image: {} } } };
