...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("CLAWDBRAIN_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  process.env.CLAWDBRAIN_STATE_DIR = path.join(base, ".clawdbrain");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "clawdbrain-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".clawdbrain", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
