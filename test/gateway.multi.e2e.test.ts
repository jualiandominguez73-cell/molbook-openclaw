...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `clawdbrain-e2e-${name}-`));
  const configDir = path.join(homeDir, ".clawdbrain");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          HOME: homeDir,
          CLAWDBRAIN_CONFIG_PATH: configPath,
          CLAWDBRAIN_STATE_DIR: stateDir,
          CLAWDBRAIN_GATEWAY_TOKEN: "",
          CLAWDBRAIN_GATEWAY_PASSWORD: "",
          CLAWDBRAIN_SKIP_CHANNELS: "1",
          CLAWDBRAIN_SKIP_BROWSER_CONTROL_SERVER: "1",
...
const waitForNodeStatus = async (inst: GatewayInstance, nodeId: string, timeoutMs = 10_000) => {
  ...
  while (Date.now() < deadline) {
    const list = (await runCliJson(
      ...
      {
        CLAWDBRAIN_GATEWAY_TOKEN: inst.gatewayToken,
        CLAWDBRAIN_GATEWAY_PASSWORD: "",
      },
    )) as NodeListPayload;
    ...
    async () => {
      ...
      const [healthA, healthB] = (await Promise.all([
        runCliJson(["health", "--json", "--timeout", "10000"], {
          CLAWDBRAIN_GATEWAY_PORT: String(gwA.port),
          CLAWDBRAIN_GATEWAY_TOKEN: gwA.gatewayToken,
          CLAWDBRAIN_GATEWAY_PASSWORD: "",
        }),
        runCliJson(["health", "--json", "--timeout", "10000"], {
          CLAWDBRAIN_GATEWAY_PORT: String(gwB.port),
          CLAWDBRAIN_GATEWAY_TOKEN: gwB.gatewayToken,
          CLAWDBRAIN_GATEWAY_PASSWORD: "",
        }),
