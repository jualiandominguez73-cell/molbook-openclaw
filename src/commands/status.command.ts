...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Clawdbrain status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("clawdbrain security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("clawdbrain security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.clawdbrain.bot/faq");
  runtime.log("Troubleshooting: https://docs.clawdbrain.bot/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("clawdbrain status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("clawdbrain logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("clawdbrain status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("clawdbrain gateway probe")}`);
  }
}
