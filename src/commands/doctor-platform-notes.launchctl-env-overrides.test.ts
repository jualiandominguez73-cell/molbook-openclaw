...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "CLAWDBRAIN_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("CLAWDBRAIN_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv CLAWDBRAIN_GATEWAY_TOKEN");
    expect(message).not.toContain("CLAWDBRAIN_GATEWAY_PASSWORD");
  });
