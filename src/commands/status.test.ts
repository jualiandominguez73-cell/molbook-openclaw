...
beforeAll(() => {
  previousProfile = process.env.CLAWDBRAIN_PROFILE;
  process.env.CLAWDBRAIN_PROFILE = "isolated";
});
...
afterAll(() => {
  if (previousProfile === undefined) {
    ...
    process.env.CLAWDBRAIN_PROFILE = previousProfile;
  }
});
...
vi.mock("../infra/clawdbrain-root.js", () => ({
  resolveClawdbrainPackageRoot: vi.fn().mockResolvedValue("/tmp/clawdbrain"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/clawdbrain/pnpm-lock.yaml",
      markerPath: "/tmp/clawdbrain/node_modules/.modules.yaml",
    },
...
vi.mock("../daemon/service.js", () => ({
  resolveGatewayService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.clawdbrain.gateway.plist",
    }),
  }),
}));
vi.mock("../daemon/node-service.js", () => ({
  resolveNodeService: () => ({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/com.clawdbrain.node.plist",
    }),
  }),
}));
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    expect(logs.some((l) => l.includes("Clawdbrain status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l) =>
          l.includes("clawdbrain status --all") ||
          l.includes("clawdbrain --profile isolated status --all") ||
          l.includes("clawdbrain status --all") ||
          l.includes("clawdbrain --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.CLAWDBRAIN_GATEWAY_TOKEN = "abcd1234";
    try {
