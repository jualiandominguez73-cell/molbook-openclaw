...
export async function maybeRepairGatewayDaemon(params: {
  ...
  if (process.platform === "darwin" && params.cfg.gateway?.mode !== "remote") {
    ...
    await maybeRepairLaunchAgentBootstrap({
      env: { ...process.env, CLAWDBRAIN_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel() },
      title: "Node",
      runtime: params.runtime,
      prompter: params.prompter,
    });
    ...
    if (params.cfg.gateway?.mode !== "remote") {
      ...
      if (install) {
        ...
        const { programArguments, workingDirectory, environment } = await buildGatewayInstallPlan({
          env: process.env,
          port,
          token: params.cfg.gateway?.auth?.token ?? process.env.CLAWDBRAIN_GATEWAY_TOKEN,
  ...
  if (process.platform === "darwin") {
    const label = resolveGatewayLaunchAgentLabel(process.env.CLAWDBRAIN_PROFILE);
    note(
      `LaunchAgent loaded; stopping requires "${formatCliCommand("clawdbrain gateway stop")}" or launchctl bootout gui/$UID/${label}.`,
      "Gateway",
    );
  }
