...
function resolveMode(cfg: ClawdbrainConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("Clawdbrain doctor");

  const root = await resolveClawdbrainPackageRoot({
  ...
  let cfg: ClawdbrainConfig = configResult.cfg;

  const configPath = configResult.path ?? CONFIG_PATH;
  if (!cfg.gateway?.mode) {
    const lines = [
      "gateway.mode is unset; gateway start will be blocked.",
      `Fix: run ${formatCliCommand("clawdbrain configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("clawdbrain config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("clawdbrain setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("clawdbrain doctor --fix")}" to apply changes.`);
  }
