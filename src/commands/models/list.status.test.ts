...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveClawdbrainAgentDir: vi.fn().mockReturnValue("/tmp/clawdbrain-agent"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveClawdbrainAgentDir: mocks.resolveClawdbrainAgentDir,
}));
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.auth.storePath).toBe("/tmp/clawdbrain-agent/auth-profiles.json");
