...
import { readConfigFileSnapshot, type ClawdbrainConfig } from "../../config/config.js";
...
function applyDefaultModel(cfg: ClawdbrainConfig, model: string): ClawdbrainConfig {
...
export async function modelsAuthLoginCommand(opts: LoginOptions, runtime: RuntimeEnv) {
  ...
  if (providers.length === 0) {
    throw new Error(
      `No provider plugins found. Install one via \`${formatCliCommand("clawdbrain plugins install")}\`.`,
    );
  }
