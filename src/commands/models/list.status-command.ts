...
export async function modelsStatusCommand(
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("clawdbrain models auth setup-token")}\` or \`${formatCliCommand("clawdbrain configure")}\`.`
          : `Run \`${formatCliCommand("clawdbrain configure")}\` or set an API key env var.`;
