...
function pruneAuthProfiles(
  cfg: ClawdbrainConfig,
  profileIds: Set<string>,
): { next: ClawdbrainConfig; changed: boolean } {
...
export async function maybeRemoveDeprecatedCliAuthProfiles(
  ...
  if (deprecated.has(CLAUDE_CLI_PROFILE_ID)) {
    lines.push(
      `- ${CLAUDE_CLI_PROFILE_ID} (Anthropic): use setup-token → ${formatCliCommand("clawdbrain models auth setup-token")}`,
    );
  }
...
function formatAuthIssueHint(issue: AuthIssue): string | null {
  if (issue.provider === "anthropic" && issue.profileId === CLAUDE_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand("clawdbrain models auth setup-token")} or ${formatCliCommand(
  ...
  if (issue.provider === "openai-codex" && issue.profileId === CODEX_CLI_PROFILE_ID) {
    return `Deprecated profile. Use ${formatCliCommand(
      "clawdbrain models auth login --provider openai-codex",
    )} or ${formatCliCommand("clawdbrain configure")}.`;
  }
  return `Re-auth via \`${formatCliCommand("clawdbrain configure")}\` or \`${formatCliCommand("clawdbrain onboard")}\`.`;
}
