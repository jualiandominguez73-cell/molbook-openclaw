...
export function listAgentEntries(cfg: ClawdbrainConfig): AgentEntry[] {
...
function resolveAgentName(cfg: ClawdbrainConfig, agentId: string) {
...
function resolveAgentModel(cfg: ClawdbrainConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: ClawdbrainConfig): AgentSummary[] {
