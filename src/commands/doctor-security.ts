...
import type { ClawdbrainConfig, GatewayBindMode } from "../config/config.js";
...
export async function noteSecurityWarnings(cfg: ClawdbrainConfig) {
  const warnings: string[] = [];
  const auditHint = `- Run: ${formatCliCommand("clawdbrain security audit --deep")}`;
    ...
    if (!hasSharedSecret) {
      const authFixLines =
        resolvedAuth.mode === "password"
          ? [
              `  Fix: ${formatCliCommand("clawdbrain configure")} to set a password`,
              `  Or switch to token: ${formatCliCommand("clawdbrain config set gateway.auth.mode token")}`,
            ]
          : [
              `  Fix: ${formatCliCommand("clawdbrain doctor --fix")} to generate a token`,
      ...
      warnings.push(
        ...
        `  Fix: ${formatCliCommand("clawdbrain config set gateway.bind loopback")}`,
        ...authFixLines,
      );
    } else {
