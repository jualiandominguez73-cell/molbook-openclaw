...
function stripUnknownConfigKeys(config: ClawdbrainConfig): {
  config: ClawdbrainConfig;
  removed: string[];
} {
  const parsed = ClawdbrainSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }

  const next = structuredClone(config) as ClawdbrainConfig;
...
function noteOpencodeProviderOverrides(cfg: ClawdbrainConfig) {
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  let cfg: ClawdbrainConfig = baseCfg;
  let candidate = structuredClone(baseCfg) as ClawdbrainConfig;
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(
        `Run "${formatCliCommand("clawdbrain doctor --fix")}" to apply legacy migrations.`,
      );
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("clawdbrain doctor --fix")}" to apply these changes.`);
    }
  }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "clawdbrain doctor --fix" to remove these keys.');
    }
  }
