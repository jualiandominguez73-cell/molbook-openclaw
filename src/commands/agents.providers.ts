...
function resolveDefaultAccountId(cfg: ClawdbrainConfig, provider: ChannelId): string {
  const plugin = getChannelPlugin(provider);
  if (!plugin) return DEFAULT_ACCOUNT_ID;
  return resolveChannelDefaultAccountId({ plugin, cfg });
}

function shouldShowProviderEntry(entry: ProviderAccountStatus, cfg: ClawdbrainConfig): boolean {
...
export function summarizeBindings(cfg: ClawdbrainConfig, bindings: AgentBinding[]): string[] {
