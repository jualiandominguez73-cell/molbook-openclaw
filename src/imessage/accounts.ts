...
export function listIMessageAccountIds(cfg: ClawdbrainConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultIMessageAccountId(cfg: ClawdbrainConfig): string {
...
export function listEnabledIMessageAccounts(cfg: ClawdbrainConfig): ResolvedIMessageAccount[] {
