...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.CLAWDBRAIN_LAUNCHD_LABEL?.trim();
  if (envLabel) return envLabel;
  return resolveGatewayLaunchAgentLabel(args?.env?.CLAWDBRAIN_PROFILE);
}
...
export function resolveGatewayLogPaths(env: Record<string, string | undefined>): {
  ...
  const prefix = env.CLAWDBRAIN_LOG_PREFIX?.trim() || "gateway";
  return {
    logDir,
