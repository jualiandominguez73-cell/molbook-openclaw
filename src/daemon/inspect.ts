...
const EXTRA_MARKERS = ["clawdbrain"];
const execFileAsync = promisify(execFile);

export function renderGatewayServiceCleanupHints(
  env: Record<string, string | undefined> = process.env as Record<string, string | undefined>,
): string[] {
  const profile = env.CLAWDBRAIN_PROFILE;
...
function hasGatewayServiceMarker(content: string): boolean {
  const lower = content.toLowerCase();
  return (
    lower.includes("clawdbrain_service_marker") &&
    lower.includes(GATEWAY_SERVICE_MARKER.toLowerCase()) &&
    lower.includes("clawdbrain_service_kind") &&
    lower.includes(GATEWAY_SERVICE_KIND.toLowerCase())
  );
}

function isClawdbrainGatewayLaunchdService(label: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) return true;
  const lowerContents = contents.toLowerCase();
  if (!lowerContents.includes("gateway")) return false;
  return label.startsWith("com.clawdbrain.");
}

function isClawdbrainGatewaySystemdService(name: string, contents: string): boolean {
  if (hasGatewayServiceMarker(contents)) return true;
  if (!name.startsWith("clawdbrain-gateway")) return false;
  return contents.toLowerCase().includes("gateway");
}

function isClawdbrainGatewayTaskName(name: string): boolean {
  const normalized = name.trim().toLowerCase();
  if (!normalized) return false;
  const defaultName = resolveGatewayWindowsTaskName().toLowerCase();
  return normalized === defaultName || normalized.startsWith("clawdbrain gateway");
}
...
async function scanLaunchdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (isClawdbrainGatewayLaunchdService(label, contents)) continue;
    results.push({
      platform: "darwin",
      label,
...
async function scanSystemdDir(params: {
  ...
  for (const entry of entries) {
    ...
    if (!containsMarker(contents)) continue;
    if (isClawdbrainGatewaySystemdService(name, contents)) continue;
    results.push({
      platform: "linux",
      label: entry,
...
export async function findExtraGatewayServices(
  ...
  if (process.platform === "win32") {
    ...
    for (const task of tasks) {
      ...
      if (!name) continue;
      if (isClawdbrainGatewayTaskName(name)) continue;
