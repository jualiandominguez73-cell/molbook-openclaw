...
function resolveTaskName(env: Record<string, string | undefined>): string {
  const override = env.CLAWDBRAIN_WINDOWS_TASK_NAME?.trim();
  if (override) return override;
  return resolveGatewayWindowsTaskName(env.CLAWDBRAIN_PROFILE);
}

export function resolveTaskScriptPath(env: Record<string, string | undefined>): string {
  const override = env.CLAWDBRAIN_TASK_SCRIPT?.trim();
  if (override) return override;
  const scriptName = env.CLAWDBRAIN_TASK_SCRIPT_NAME?.trim() || "gateway.cmd";
...
export async function installScheduledTask({
  ...
  const taskDescription =
    ...
    formatGatewayServiceDescription({
      profile: env.CLAWDBRAIN_PROFILE,
      version: environment?.CLAWDBRAIN_SERVICE_VERSION ?? env.CLAWDBRAIN_SERVICE_VERSION,
    });
