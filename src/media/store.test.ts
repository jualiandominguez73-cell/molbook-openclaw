...
describe("media store", () => {
  ...
  const snapshotEnv = () => {
    for (const key of ["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "CLAWDBRAIN_STATE_DIR"]) {
  ...
  beforeAll(async () => {
    snapshotEnv();
    home = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-test-home-"));
    process.env.HOME = home;
    process.env.USERPROFILE = home;
    process.env.CLAWDBRAIN_STATE_DIR = path.join(home, ".clawdbrain");
    ...
    await fs.mkdir(path.join(home, ".clawdbrain"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.clawdbrain${path.sep}media`);
