    ...
    try {
...

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "clawdbrain", messages: [{ role: "user", content: "hi" }] },
          { "x-clawdbrain-agent-id": "beta" },
        );
...

      {
        ...
        const res = await postChatCompletions(
          ...
          {
            "x-clawdbrain-agent-id": "beta",
            "x-clawdbrain-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
