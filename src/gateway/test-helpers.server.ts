...
async function setupGatewayTestHome() {
  ...
  previousStateDir = process.env.CLAWDBRAIN_STATE_DIR;
  previousConfigPath = process.env.CLAWDBRAIN_CONFIG_PATH;
  previousSkipBrowserControl = process.env.CLAWDBRAIN_SKIP_BROWSER_CONTROL_SERVER;
  previousSkipGmailWatcher = process.env.CLAWDBRAIN_SKIP_GMAIL_WATCHER;
  previousSkipCanvasHost = process.env.CLAWDBRAIN_SKIP_CANVAS_HOST;
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-gateway-home-"));
  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.CLAWDBRAIN_STATE_DIR = path.join(tempHome, ".clawdbrain");
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  tempConfigRoot = options.uniqueConfigRoot
    ? await fs.mkdtemp(path.join(tempHome, "clawdbrain-test-"))
    : path.join(tempHome, ".clawdbrain-test");
...
async function cleanupGatewayTestHome(options: { restoreEnv: boolean }) {
  ...
  if (options.restoreEnv) {
    ...
    if (previousStateDir === undefined) delete process.env.CLAWDBRAIN_STATE_DIR;
    else process.env.CLAWDBRAIN_STATE_DIR = previousStateDir;
    if (previousConfigPath === undefined) delete process.env.CLAWDBRAIN_CONFIG_PATH;
    else process.env.CLAWDBRAIN_CONFIG_PATH = previousConfigPath;
    if (previousSkipBrowserControl === undefined)
      delete process.env.CLAWDBRAIN_SKIP_BROWSER_CONTROL_SERVER;
    else process.env.CLAWDBRAIN_SKIP_BROWSER_CONTROL_SERVER = previousSkipBrowserControl;
    if (previousSkipGmailWatcher === undefined) delete process.env.CLAWDBRAIN_SKIP_GMAIL_WATCHER;
    else process.env.CLAWDBRAIN_SKIP_GMAIL_WATCHER = previousSkipGmailWatcher;
    if (previousSkipCanvasHost === undefined) delete process.env.CLAWDBRAIN_SKIP_CANVAS_HOST;
    else process.env.CLAWDBRAIN_SKIP_CANVAS_HOST = previousSkipCanvasHost;
  }
...
export async function startServerWithClient(token?: string, opts?: GatewayServerOptions) {
  ...
  const prev = process.env.CLAWDBRAIN_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.CLAWDBRAIN_GATEWAY_TOKEN = fallbackToken;
  }
