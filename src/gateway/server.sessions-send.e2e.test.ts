...
import { createClawdbrainTools } from "../agents/clawdbrain-tools.js";
...
beforeAll(async () => {
  prevGatewayPort = process.env.CLAWDBRAIN_GATEWAY_PORT;
  prevGatewayToken = process.env.CLAWDBRAIN_GATEWAY_TOKEN;
  gatewayPort = await getFreePort();
  process.env.CLAWDBRAIN_GATEWAY_PORT = String(gatewayPort);
  process.env.CLAWDBRAIN_GATEWAY_TOKEN = "test-token";
...
afterAll(async () => {
  ...
  if (prevGatewayPort === undefined) {
    ...
    process.env.CLAWDBRAIN_GATEWAY_PORT = prevGatewayPort;
  }
  if (prevGatewayToken === undefined) {
    delete process.env.CLAWDBRAIN_GATEWAY_TOKEN;
  } else {
    process.env.CLAWDBRAIN_GATEWAY_TOKEN = prevGatewayToken;
  }
});
...
describe("sessions_send gateway loopback", () => {
  it("returns reply when lifecycle ends before agent.wait", async () => {
    ...
    const tool = createClawdbrainTools().find((candidate) => candidate.name === "sessions_send");
