...
describe("gateway e2e", () => {
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-gw-mock-home-"));
      ...
      const toolProbePath = path.join(workspaceDir, `.clawdbrain-tool-probe.${nonceA}.txt`);
  ...
  it("runs wizard over ws and writes auth token config", { timeout: 90_000 }, async () => {
    ...
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-wizard-home-"));
    process.env.HOME = tempHome;
