...
export function resolveHooksConfig(cfg: ClawdbrainConfig): HooksConfigResolved | null {
...
export function extractHookToken(req: IncomingMessage, url: URL): HookTokenResult {
  ...
  const headerToken =
    typeof req.headers["x-clawdbrain-token"] === "string"
      ? req.headers["x-clawdbrain-token"].trim()
      : "";
