...
export function formatBonjourInstanceName(displayName: string) {
  ...
  if (!trimmed) return "Clawdbrain";
  if (/clawdbrain/i.test(trimmed)) return trimmed;
  return `${trimmed} (Clawdbrain)`;
}

export function resolveBonjourCliPath(opts: ResolveBonjourCliPathOptions = {}): string | undefined {
  const env = opts.env ?? process.env;
  const envPath = env.CLAWDBRAIN_CLI_PATH?.trim();
  ...
  const siblingCli = path.join(execDir, "clawdbrain");
  ...
  const binCli = path.join(cwd, "bin", "clawdbrain.js");
...
export async function resolveTailnetDnsHint(opts?: {
  ...
  const env = opts?.env ?? process.env;
  const envRaw = env.CLAWDBRAIN_TAILNET_DNS?.trim();
