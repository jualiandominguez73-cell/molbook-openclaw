...
import { resolveClawdbrainAgentDir } from "../agents/agent-paths.js";
...
import { ensureClawdbrainModelsJson } from "../agents/models-config.js";
import { loadConfig } from "../config/config.js";
import type { ClawdbrainConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.CLAWDBRAIN_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.CLAWDBRAIN_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.CLAWDBRAIN_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: ClawdbrainConfig;
  agentDir: string;
}): ClawdbrainConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<ClawdbrainConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.CLAWDBRAIN_AGENT_DIR,
  ...
  const hostAgentDir = resolveClawdbrainAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-live-state-"));
  ...
  process.env.CLAWDBRAIN_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.clawdbrain-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveClawdbrainAgentDir();
  const sanitizedCfg: ClawdbrainConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "Clawdbrain live tool probe (local, safe): " +
    ...
    process.env.CLAWDBRAIN_CONFIG_PATH = previous.configPath;
    process.env.CLAWDBRAIN_GATEWAY_TOKEN = previous.token;
    process.env.CLAWDBRAIN_SKIP_CHANNELS = previous.skipChannels;
    process.env.CLAWDBRAIN_SKIP_GMAIL_WATCHER = previous.skipGmail;
    process.env.CLAWDBRAIN_SKIP_CRON = previous.skipCron;
    process.env.CLAWDBRAIN_SKIP_CANVAS_HOST = previous.skipCanvas;
    process.env.CLAWDBRAIN_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.CLAWDBRAIN_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      const rawModels = process.env.CLAWDBRAIN_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.clawdbrain-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
