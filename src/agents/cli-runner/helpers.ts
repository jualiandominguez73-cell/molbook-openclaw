...
/**
 * Cleanup suspended Clawdbrain CLI processes that have accumulated.
...
function buildModelAliasLines(cfg?: ClawdbrainConfig) {
...
export async function writeCliImages(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-cli-images-"));
  const paths: string[] = [];
