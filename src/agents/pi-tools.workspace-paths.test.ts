...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("clawdbrain-ws-", async (workspaceDir) => {
      await withTempDir("clawdbrain-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createClawdbrainCodingTools({ workspaceDir });
    ...
    await withTempDir("clawdbrain-ws-", async (workspaceDir) => {
      await withTempDir("clawdbrain-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello clawdbrain");
        } finally {
          process.chdir(prevCwd);
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("clawdbrain-ws-", async (workspaceDir) => {
      await withTempDir("clawdbrain-override-", async (overrideDir) => {
        const tools = createClawdbrainCodingTools({ workspaceDir });
        const execTool = tools.find((tool) => tool.name === "exec");
        expect(execTool).toBeDefined();
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("clawdbrain-sandbox-", async (sandboxDir) => {
      await withTempDir("clawdbrain-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
          ...
          workspaceAccess: "rw",
          containerName: "clawdbrain-sbx-test",
        ...
        const tools = createClawdbrainCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
