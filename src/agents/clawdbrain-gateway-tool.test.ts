...
describe("gateway tool", () => {
  it("schedules SIGUSR1 restart", async () => {
    ...
    const previousProfile = process.env.CLAWDBRAIN_PROFILE;
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-test-"));
    process.env.CLAWDBRAIN_STATE_DIR = stateDir;
    process.env.CLAWDBRAIN_PROFILE = "isolated";

    try {
      const tool = createClawdbrainTools({
      ...
      expect(parsed.payload?.doctorHint).toBe(
        "Run: clawdbrain --profile isolated doctor --non-interactive",
      );
