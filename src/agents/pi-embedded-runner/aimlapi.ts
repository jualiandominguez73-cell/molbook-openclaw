...
>(params: {
  ...
  const normalizedTools = params.tools.map((tool) => {
    ...
    if (
      ...
      transformedTools += 1;
      log.warn("aimlapi tools: schema transformed for compatibility", {
        tool: tool.name,
        ...summary,
      });
    }
  ...
  if (transformedTools > 0) {
    log.warn("aimlapi tools: completed schema compatibility pass", {
      transformedTools,
...
function rewriteAssistantNullContentInMessages(params: {
  ...
  for (const [index, entry] of params.messages.entries()) {
    ...
    record.content = "";
    replaced += 1;
    log.warn("aimlapi message normalization: rewrote assistant null content", {
  ...
  if (replaced > 0) {
    log.warn("aimlapi message normalization: rewrite summary", {
      source: params.source,
      replaced,
...
export function normalizeAimlapiAssistantNullContent(params: {
  ...
  const nextMessages = params.messages.map((message, index) => {
    ...
    replacedCount += 1;
    log.warn("aimlapi message normalization: replacing assistant null content with empty string", {
      index,
  ...
  if (replacedCount > 0) {
    log.warn("aimlapi message normalization: replaced assistant null content entries", {
      replacedCount,
      totalMessages: params.messages.length,
    });
  }
...
export function rollbackFailedAimlapiPrompt(params: {
  ...
  log.warn("aimlapi invalid schema: attempting prompt rollback", {
    runId: params.runId,
  ...
  if (leafEntry?.type === "message" && leafEntry.message.role === "user") {
    if (leafEntry.parentId) {
      log.info("aimlapi invalid schema: rolling back by branching to parent", {
        runId: params.runId,
        sessionId: params.sessionId,
      ...
      log.info("aimlapi invalid schema: rolling back by resetting leaf", {
        runId: params.runId,
        sessionId: params.sessionId,
      });
    ...
    log.warn("aimlapi invalid schema: removed failed prompt turn from persisted history", {
      runId: params.runId,
      sessionId: params.sessionId,
  ...
  if (params.activeMessages[params.activeMessages.length - 1]?.role === "user") {
    ...
    log.warn("aimlapi invalid schema: trimmed last transient user turn in memory", {
      runId: params.runId,
      sessionId: params.sessionId,
  ...
  log.warn("aimlapi invalid schema: rollback attempted but no removable user turn found", {
    runId: params.runId,
    sessionId: params.sessionId,
    activeMessages: params.activeMessages.length,
