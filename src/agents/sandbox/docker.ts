...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "clawdbrain.sandbox=1");
  args.push("--label", `clawdbrain.sessionKey=${params.scopeKey}`);
  args.push("--label", `clawdbrain.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `clawdbrain.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  const result = await execDocker(
    ["inspect", "-f", '{{ index .Config.Labels "clawdbrain.configHash" }}', containerName],
    { allowFailure: true },
  );
  if (result.code !== 0) return null;
...
function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`clawdbrain sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`clawdbrain sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("clawdbrain sandbox recreate --all");
}
