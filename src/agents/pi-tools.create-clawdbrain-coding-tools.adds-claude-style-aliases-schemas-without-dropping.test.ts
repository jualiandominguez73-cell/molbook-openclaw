...
import { __testing, createClawdbrainCodingTools } from "./pi-tools.js";
...
describe("createClawdbrainCodingTools", () => {
  ...
  it("keeps raw core tool schemas union-free", () => {
    const tools = createClawdbrainTools();
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createClawdbrainCodingTools({ messageProvider: "discord" });
  ...
  it("applies sandbox path guards to file_path alias", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-sbx-"));
    const outsidePath = path.join(os.tmpdir(), "clawdbrain-outside.txt");
