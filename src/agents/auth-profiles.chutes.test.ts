...
describe("auth-profiles (chutes)", () => {
  ...
  afterEach(async () => {
    ...
    if (previousAgentDir === undefined) delete process.env.CLAWDBRAIN_AGENT_DIR;
    else process.env.CLAWDBRAIN_AGENT_DIR = previousAgentDir;
  ...
  it("refreshes expired Chutes OAuth credentials", async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-chutes-"));
    process.env.CLAWDBRAIN_STATE_DIR = tempDir;
    process.env.CLAWDBRAIN_AGENT_DIR = path.join(tempDir, "agents", "main", "agent");
