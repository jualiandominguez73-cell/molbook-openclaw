...
describe("createClawdbrainCodingTools", () => {
  it("uses workspaceDir for Read tool path resolution", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-ws-"));
    try {
      ...
      const tools = createClawdbrainCodingTools({ workspaceDir: tmpDir });
  ...
  it("accepts Claude Code parameter aliases for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-alias-"));
    try {
      const tools = createClawdbrainCodingTools({ workspaceDir: tmpDir });
      const readTool = tools.find((tool) => tool.name === "read");
      const writeTool = tools.find((tool) => tool.name === "write");
