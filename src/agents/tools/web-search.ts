...
type WebSearchConfig = NonNullable<ClawdbrainConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: ClawdbrainConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("clawdbrain configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.clawdbrain.bot/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://clawdbrain.bot",
      "X-Title": "Clawdbrain Web Search",
    },
    body: JSON.stringify({
      model: params.model,
      messages: [
