...
const defaultTools = createClawdbrainCodingTools();

describe("createClawdbrainCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    const readTool = defaultTools.find((tool) => tool.name === "read");
    expect(readTool).toBeDefined();

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    const tools = createClawdbrainCodingTools();
    ...
    try {
      ...
      const contents = "Hello from clawdbrain read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandbox = {
      ...
      workspaceDir: path.join(os.tmpdir(), "clawdbrain-sandbox"),
      agentWorkspaceDir: path.join(os.tmpdir(), "clawdbrain-workspace"),
    ...
    const tools = createClawdbrainCodingTools({ sandbox });
