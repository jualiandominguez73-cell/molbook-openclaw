...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "clawdbrain-cron-" });
}

async function writeSessionStore(home: string) {
  const dir = path.join(home, ".clawdbrain", "sessions");
...
function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<ClawdbrainConfig> = {},
): ClawdbrainConfig {
  const base: ClawdbrainConfig = {
    agents: {
      defaults: {
...
describe("runCronIsolatedAgentTurn", () => {
  ...
  it("uses agentId for workspace, session key, and store paths", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = makeCfg(
        home,
        path.join(home, ".clawdbrain", "agents", "{agentId}", "sessions", "sessions.json"),
        {
          agents: {
