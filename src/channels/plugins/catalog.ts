...
import { discoverClawdbrainPlugins } from "../../plugins/discovery.js";
import type { PluginOrigin } from "../../plugins/types.js";
import type { ClawdbrainPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
  clawdbrain?: ClawdbrainPackageManifest;
  [LEGACY_MANIFEST_KEY]?: ClawdbrainPackageManifest;
};
...
const ENV_CATALOG_PATHS = ["CLAWDBRAIN_PLUGIN_CATALOG_PATHS", "CLAWDBRAIN_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<ClawdbrainPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: ClawdbrainPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageClawdbrain?: ClawdbrainPackageManifest;
}): ChannelPluginCatalogEntry | null {
  const manifest = candidate.packageClawdbrain;
...
function buildExternalCatalogEntry(entry: ExternalCatalogEntry): ChannelPluginCatalogEntry | null {
  const manifest = entry.clawdbrain ?? entry[LEGACY_MANIFEST_KEY];
  return buildCatalogEntry({
    packageName: entry.name,
    packageClawdbrain: manifest,
  });
}
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverClawdbrainPlugins({ workspaceDir: options.workspaceDir });
