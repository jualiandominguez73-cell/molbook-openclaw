...
import type { ClawdbrainConfig, ConfigFileSnapshot } from "../config/config.js";
...
function summarizeGroupPolicy(cfg: ClawdbrainConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
export function collectAttackSurfaceSummaryFindings(cfg: ClawdbrainConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep CLAWDBRAIN_STATE_DIR on a local-only volume and re-run "${formatCliCommand("clawdbrain security audit --fix")}".`,
    });
  }
  return findings;
}
...
export function collectSecretsInConfigFindings(cfg: ClawdbrainConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer CLAWDBRAIN_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(cfg: ClawdbrainConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectModels(cfg: ClawdbrainConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
export function collectModelHygieneFindings(cfg: ClawdbrainConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function hasWebSearchKey(cfg: ClawdbrainConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: ClawdbrainConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: ClawdbrainConfig): boolean {
  const enabled = cfg.tools?.web?.fetch?.enabled;
  if (enabled === false) return false;
  return true;
}

function isBrowserEnabled(cfg: ClawdbrainConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: ClawdbrainConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
...
export function collectExposureMatrixFindings(cfg: ClawdbrainConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
