...
export function listSignalAccountIds(cfg: ClawdbrainConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSignalAccountId(cfg: ClawdbrainConfig): string {
...
function mergeSignalAccountConfig(cfg: ClawdbrainConfig, accountId: string): SignalAccountConfig {
...
export function listEnabledSignalAccounts(cfg: ClawdbrainConfig): ResolvedSignalAccount[] {
