...
import { resolveClawdbrainPackageRoot } from "../infra/clawdbrain-root.js";
import {
  checkUpdateStatus,
...
const STEP_LABELS: Record<string, string> = {
  ...
  "clawdbrain doctor": "Running doctor checks",
...
const DEFAULT_PACKAGE_NAME = "clawdbrain";
const CORE_PACKAGE_NAMES = new Set([DEFAULT_PACKAGE_NAME, "clawdbrain"]);
const CLI_NAME = resolveCliName();
const CLAWDBRAIN_REPO_URL = "https://github.com/clawdbrain/clawdbrain.git";
const DEFAULT_GIT_DIR = path.join(os.homedir(), "clawdbrain");

function normalizeTag(value?: string | null): string | null {
  if (!value) return null;
  const trimmed = value.trim();
  if (!trimmed) return null;
  if (trimmed.startsWith("clawdbrain@")) return trimmed.slice("clawdbrain@".length);
...
function resolveGitInstallDir(): string {
  const override = process.env.CLAWDBRAIN_GIT_DIR?.trim();
...
async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", CLAWDBRAIN_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `CLAWDBRAIN_GIT_DIR points at a non-git directory: ${params.dir}. Set CLAWDBRAIN_GIT_DIR to an empty folder or a clawdbrain checkout.`,
      );
    }
    return await runUpdateStep({
      name: "git clone",
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`CLAWDBRAIN_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function updateStatusCommand(opts: UpdateStatusOptions): Promise<void> {
  ...
  const root =
    (await resolveClawdbrainPackageRoot({
  ...
  defaultRuntime.log(theme.heading("Clawdbrain update status"));
  defaultRuntime.log("");
  defaultRuntime.log(
    renderTable({
      width: tableWidth,
...
export async function updateCommand(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (!opts.json) {
    defaultRuntime.log(theme.heading("Updating Clawdbrain..."));
    defaultRuntime.log("");
  }
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "not-git-install") {
      defaultRuntime.log(
        theme.warn(
          `Skipped: this Clawdbrain install isn't a git checkout, and the package manager couldn't be detected. Update via your package manager, then run \`${replaceCliName(formatCliCommand("clawdbrain doctor"), CLI_NAME)}\` and \`${replaceCliName(formatCliCommand("clawdbrain gateway restart"), CLI_NAME)}\`.`,
        ),
      );
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g clawdbrain@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g clawdbrain@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (shouldRestart) {
    ...
    try {
      ...
      if (!opts.json && restarted) {
        ...
        defaultRuntime.log("");
        process.env.CLAWDBRAIN_UPDATE_IN_PROGRESS = "1";
        try {
          ...
          delete process.env.CLAWDBRAIN_UPDATE_IN_PROGRESS;
        }
      }
    } catch (err) {
      if (!opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("clawdbrain gateway restart"), CLI_NAME)}`,
          ),
        );
      }
    }
    ...
    if (result.mode === "npm" || result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("clawdbrain doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("clawdbrain gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("clawdbrain gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
export async function updateWizardCommand(opts: UpdateWizardOptions = {}): Promise<void> {
  if (!process.stdin.isTTY) {
    defaultRuntime.error(
      "Update wizard requires a TTY. Use `clawdbrain update --channel <stable|beta|dev>` instead.",
    );
    defaultRuntime.exit(1);
    return;
  }
  ...
  if (requestedChannel === "dev" && updateStatus.installKind !== "git") {
    ...
    if (!hasGit) {
      ...
      if (dirExists) {
        ...
        if (!empty) {
          defaultRuntime.error(
            `CLAWDBRAIN_GIT_DIR points at a non-git directory: ${gitDir}. Set CLAWDBRAIN_GIT_DIR to an empty folder or a clawdbrain checkout.`,
          );
          defaultRuntime.exit(1);
          return;
        }
      ...
      const ok = await confirm({
        message: stylePromptMessage(
          `Create a git checkout at ${gitDir}? (override via CLAWDBRAIN_GIT_DIR)`,
        ),
        initialValue: true,
      });
      if (isCancel(ok) || ok === false) {
...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Clawdbrain to the latest version")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["clawdbrain update", "Update a source checkout (git)"],
        ["clawdbrain update --channel beta", "Switch to beta channel (git + npm)"],
        ["clawdbrain update --channel dev", "Switch to dev channel (git + npm)"],
        ["clawdbrain update --tag beta", "One-off update to a dist-tag or version"],
        ["clawdbrain update --no-restart", "Update without restarting the service"],
        ["clawdbrain update --json", "Output result as JSON"],
        ["clawdbrain update --yes", "Non-interactive (accept downgrade prompts)"],
        ["clawdbrain update wizard", "Interactive update wizard"],
        ["clawdbrain --update", "Shorthand for clawdbrain update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run clawdbrain update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.clawdbrain.bot/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["clawdbrain update status", "Show channel + version status."],
          ["clawdbrain update status --json", "JSON output."],
          ["clawdbrain update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.clawdbrain.bot/cli/update")}`,
    )
    .action(async (opts) => {
      try {
