...
function renderNodeServiceStartHints(): string[] {
  const base = [
    formatCliCommand("clawdbrain node install"),
    formatCliCommand("clawdbrain node start"),
  ];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("clawdbrain node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    CLAWDBRAIN_LOG_PREFIX: baseEnv.CLAWDBRAIN_LOG_PREFIX ?? "node",
