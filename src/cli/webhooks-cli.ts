...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.clawdbrain.bot/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + Clawdbrain hooks")
    ...
    .option("--hook-url <url>", "Clawdbrain hook URL")
    .option("--hook-token <token>", "Clawdbrain hook token")
