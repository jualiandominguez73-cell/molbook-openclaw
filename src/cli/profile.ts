...
function resolveProfileStateDir(profile: string, homedir: () => string): string {
  ...
  return path.join(homedir(), `.clawdbrain${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.CLAWDBRAIN_PROFILE = profile;

  const stateDir = env.CLAWDBRAIN_STATE_DIR?.trim() || resolveProfileStateDir(profile, homedir);
  if (!env.CLAWDBRAIN_STATE_DIR?.trim()) env.CLAWDBRAIN_STATE_DIR = stateDir;

  if (!env.CLAWDBRAIN_CONFIG_PATH?.trim()) {
    env.CLAWDBRAIN_CONFIG_PATH = path.join(stateDir, "clawdbrain.json");
  }

  if (profile === "dev" && !env.CLAWDBRAIN_GATEWAY_PORT?.trim()) {
    env.CLAWDBRAIN_GATEWAY_PORT = "19001";
  }
}
