...
${formatHelpExamples([
  ['clawdbrain agent --to +15555550123 --message "status update"', "Start a new session."],
  ['clawdbrain agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'clawdbrain agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'clawdbrain agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['clawdbrain agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'clawdbrain agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.clawdbrain.bot/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.clawdbrain.bot/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['clawdbrain agents set-identity --agent main --name "Clawd" --emoji "🦞"', "Set name + emoji."],
  ["clawdbrain agents set-identity --agent main --avatar avatars/clawd.png", "Set avatar path."],
  ["clawdbrain agents set-identity --workspace ~/clawd --from-identity", "Load from IDENTITY.md."],
  [
    "clawdbrain agents set-identity --identity-file ~/clawd/IDENTITY.md --agent main",
