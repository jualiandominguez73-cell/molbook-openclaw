...
function listConfiguredAccountIds(cfg: ClawdbrainConfig): string[] {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts || typeof accounts !== "object") return [];
  return Object.keys(accounts).filter(Boolean);
}

export function listWhatsAppAuthDirs(cfg: ClawdbrainConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: ClawdbrainConfig): boolean {
  return listWhatsAppAuthDirs(cfg).some((authDir) => hasWebCredsSync(authDir));
}

export function listWhatsAppAccountIds(cfg: ClawdbrainConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultWhatsAppAccountId(cfg: ClawdbrainConfig): string {
...
export function resolveWhatsAppAuthDir(params: { cfg: ClawdbrainConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: ClawdbrainConfig): ResolvedWhatsAppAccount[] {
