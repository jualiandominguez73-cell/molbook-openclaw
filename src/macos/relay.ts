...
async function main() {
  ...
  const { ensureClawdbrainCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[clawdbrain] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
