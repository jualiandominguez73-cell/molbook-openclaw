  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(
        path.join(os.tmpdir(), "clawdbrain-update-preflight-"),
      );
    ...
    const doctorStep = await runStep(
      step(
        "clawdbrain doctor",
        managerScriptArgs(manager, "clawdbrain", ["doctor", "--non-interactive"]),
        gitRoot,
        { CLAWDBRAIN_UPDATE_IN_PROGRESS: "1" },
      ),
    );
