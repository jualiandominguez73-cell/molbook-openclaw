...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: ClawdbrainConfig | null = null;

  constructor(private readonly ttlMs: number) {}

  get(key: string, cfg: ClawdbrainConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: ClawdbrainConfig): void {
    this.resetIfConfigChanged(cfg);
  ...
  clear(cfg?: ClawdbrainConfig): void {
    this.cache.clear();
    if (cfg) this.lastConfigRef = cfg;
  }

  private resetIfConfigChanged(cfg: ClawdbrainConfig): void {
