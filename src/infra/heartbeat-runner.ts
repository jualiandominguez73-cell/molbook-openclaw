...
function resolveActiveHoursTimezone(cfg: ClawdbrainConfig, raw?: string): string {
...
function hasExplicitHeartbeatAgents(cfg: ClawdbrainConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: ClawdbrainConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: ClawdbrainConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: ClawdbrainConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: ClawdbrainConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: ClawdbrainConfig) => {
    if (state.stopped) return;
    const now = Date.now();
