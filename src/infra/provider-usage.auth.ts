...
function resolveVolcengineApiKey(): string | undefined {
  const envDirect = process.env.VOLCENGINE_API_KEY?.trim();
  if (envDirect) {
    return envDirect;
  }

  const envResolved = resolveEnvApiKey("volcengine");
  if (envResolved?.apiKey) {
    return envResolved.apiKey;
  }

  const cfg = loadConfig();
  const key = getCustomProviderApiKey(cfg, "volcengine");
  if (key) {
    return key;
  }

  const store = ensureAuthProfileStore();
  const apiProfile = listProfilesForProvider(store, "volcengine").find((id) => {
    const cred = store.profiles[id];
    return cred?.type === "api_key" || cred?.type === "token";
  });
  if (!apiProfile) {
