...
export const WIDE_AREA_DISCOVERY_DOMAIN = "clawdbrain.internal.";
export const WIDE_AREA_ZONE_FILENAME = "clawdbrain.internal.db";
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*clawdbrain-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "clawdbrain";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "clawdbrain");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "clawdbrain-gw");
  ...
  records.push(`_clawdbrain-gw._tcp IN PTR ${instanceLabel}._clawdbrain-gw._tcp`);
  records.push(`${instanceLabel}._clawdbrain-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._clawdbrain-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; clawdbrain-content-hash: ${contentHash}\n${contentBody}`;
}
