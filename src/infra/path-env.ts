...
function candidateBinDirs(opts: EnsureClawdbrainPathOpts): string[] {
  ...
  // Bundled macOS app: `clawdbrain` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingClawdbrain = path.join(execDir, "clawdbrain");
    if (isExecutable(siblingClawdbrain)) candidates.push(execDir);
  } catch {
    // ignore
  }

  // Project-local installs (best effort): if a `node_modules/.bin/clawdbrain` exists near cwd,
  // include it. This helps when running under launchd or other minimal PATH environments.
  const localBinDir = path.join(cwd, "node_modules", ".bin");
  if (isExecutable(path.join(localBinDir, "clawdbrain"))) candidates.push(localBinDir);
...
/**
 * Best-effort PATH bootstrap so skills that require the `clawdbrain` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureClawdbrainCliOnPath(opts: EnsureClawdbrainPathOpts = {}) {
  if (isTruthyEnvValue(process.env.CLAWDBRAIN_PATH_BOOTSTRAPPED)) return;
  process.env.CLAWDBRAIN_PATH_BOOTSTRAPPED = "1";
