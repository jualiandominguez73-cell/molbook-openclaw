...
export function triggerClawdbrainRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.CLAWDBRAIN_SYSTEMD_UNIT,
        process.env.CLAWDBRAIN_PROFILE,
      );
  ...
  const label =
    process.env.CLAWDBRAIN_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.CLAWDBRAIN_PROFILE);
