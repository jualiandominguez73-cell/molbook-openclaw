...
export function listSlackAccountIds(cfg: ClawdbrainConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultSlackAccountId(cfg: ClawdbrainConfig): string {
...
function mergeSlackAccountConfig(cfg: ClawdbrainConfig, accountId: string): SlackAccountConfig {
...
export function listEnabledSlackAccounts(cfg: ClawdbrainConfig): ResolvedSlackAccount[] {
