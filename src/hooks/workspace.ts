...
function resolvePackageHooks(manifest: HookPackageManifest): string[] {
  const raw = manifest.clawdbrain?.hooks ?? manifest[LEGACY_MANIFEST_KEY]?.hooks;
...
export function loadHookEntriesFromDir(params: {
  ...
  return hooks.map((hook) => {
    ...
    const entry: HookEntry = {
      ...
      frontmatter,
      metadata: resolveClawdbrainMetadata(frontmatter),
...
function loadHookEntries(
  ...
  const extraHooks = extraDirs.flatMap((dir) => {
    ...
    return loadHooksFromDir({
      dir: resolved,
      source: "clawdbrain-workspace", // Extra dirs treated as workspace
    });
  });
  const managedHooks = loadHooksFromDir({
    dir: managedHooksDir,
    source: "clawdbrain-managed",
  });
  const workspaceHooks = loadHooksFromDir({
    dir: workspaceHooksDir,
    source: "clawdbrain-workspace",
  });
