...
import type { ClawdbrainPluginApi } from "../plugins/types.js";
...
function resolveHookDir(api: ClawdbrainPluginApi, dir: string): string {
  if (path.isAbsolute(dir)) return dir;
  return path.resolve(path.dirname(api.source), dir);
}

function normalizePluginHookEntry(api: ClawdbrainPluginApi, entry: HookEntry): HookEntry {
  return {
    ...entry,
    hook: {
      ...entry.hook,
