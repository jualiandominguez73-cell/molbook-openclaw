...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `clawdbrain-hook-install-${randomUUID()}`);
...
describe("installHooksFromArchive", () => {
  it("installs hook packs from zip archives", async () => {
    ...
    zip.file(
      ...
      JSON.stringify({
        name: "@clawdbrain/zip-hooks",
        version: "0.0.1",
        clawdbrain: { hooks: ["./hooks/zip-hook"] },
      }),
    );
    zip.file(
  ...
  it("installs hook packs from tar archives", async () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@clawdbrain/tar-hooks",
        version: "0.0.1",
        clawdbrain: { hooks: ["./hooks/tar-hook"] },
      }),
      "utf-8",
    );
    fs.writeFileSync(
