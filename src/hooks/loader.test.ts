...
describe("loader", () => {
  ...
  beforeEach(async () => {
    ...
    tmpDir = path.join(os.tmpdir(), `clawdbrain-test-${Date.now()}`);
    await fs.mkdir(tmpDir, { recursive: true });

    // Disable bundled hooks during tests by setting env var to non-existent directory
    originalBundledDir = process.env.CLAWDBRAIN_BUNDLED_HOOKS_DIR;
    process.env.CLAWDBRAIN_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      ...
      process.env.CLAWDBRAIN_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
