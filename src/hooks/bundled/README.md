...
This directory contains hooks that ship with Clawdbrain. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Docs**: https://docs.clawdbrain.bot/hooks/soul-evil
...
metadata:
  { "clawdbrain": { "emoji": "🔗", "events": ["command:new"], "requires": { "bins": ["node"] } } }
---
# Hook Title
...
Hooks can be configured in `~/.clawdbrain/clawdbrain.json`:
...
2. Restart gateway: `pkill -9 -f 'clawdbrain.*gateway' && pnpm clawdbrain gateway`
3. Enable the hook: `clawdbrain hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.clawdbrain.bot/hooks
