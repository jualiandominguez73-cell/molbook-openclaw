...
const execHostEnforced = process.env.CLAWDBRAIN_NODE_EXEC_HOST?.trim().toLowerCase() === "app";
const execHostFallbackAllowed =
  process.env.CLAWDBRAIN_NODE_EXEC_FALLBACK?.trim().toLowerCase() !== "0";
...
function ensureNodePathEnv(): string {
  ensureClawdbrainCliOnPath({ pathEnv: process.env.PATH ?? "" });
