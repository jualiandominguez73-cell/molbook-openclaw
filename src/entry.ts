...
function ensureExperimentalWarningSuppressed(): boolean {
  if (isTruthyEnvValue(process.env.CLAWDBRAIN_NO_RESPAWN)) return false;
  if (isTruthyEnvValue(process.env.CLAWDBRAIN_NODE_OPTIONS_READY)) return false;
  const nodeOptions = process.env.NODE_OPTIONS ?? "";
  if (hasExperimentalWarningSuppressed(nodeOptions)) return false;

  process.env.CLAWDBRAIN_NODE_OPTIONS_READY = "1";
  ...
  child.once("error", (error) => {
    console.error(
      "[clawdbrain] Failed to respawn CLI:",
...
if (!ensureExperimentalWarningSuppressed()) {
  ...
  if (!parsed.ok) {
    ...
    console.error(`[clawdbrain] ${parsed.error}`);
    process.exit(2);
  }
  ...
  import("./cli/run-main.js")
    ...
    .catch((error) => {
      console.error(
        "[clawdbrain] Failed to start CLI:",
        error instanceof Error ? (error.stack ?? error.message) : error,
      );
      process.exitCode = 1;
    });
