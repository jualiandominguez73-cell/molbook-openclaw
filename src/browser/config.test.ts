...
describe("browser config", () => {
  ...
  it("derives default ports from CLAWDBRAIN_GATEWAY_PORT when unset", () => {
    const prev = process.env.CLAWDBRAIN_GATEWAY_PORT;
    process.env.CLAWDBRAIN_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      if (prev === undefined) {
        ...
        process.env.CLAWDBRAIN_GATEWAY_PORT = prev;
      }
    }
  });
