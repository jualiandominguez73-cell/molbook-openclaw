...
import { loadClawdbrainPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const tempDirs: string[] = [];
const prevBundledDir = process.env.CLAWDBRAIN_BUNDLED_PLUGINS_DIR;
const EMPTY_PLUGIN_SCHEMA = { type: "object", additionalProperties: false, properties: {} };

function makeTempDir() {
  const dir = path.join(os.tmpdir(), `clawdbrain-plugin-${randomUUID()}`);
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "clawdbrain.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
afterEach(() => {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.CLAWDBRAIN_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});

describe("loadClawdbrainPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    process.env.CLAWDBRAIN_BUNDLED_PLUGINS_DIR = bundledDir;

    const registry = loadClawdbrainPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadClawdbrainPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
