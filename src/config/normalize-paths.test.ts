  ...
  it("expands tilde for path-ish keys only", async () => {
    await withTempHome(async (home) => {
      ...
      const cfg = normalizeConfigPaths({
        ...
        logging: { file: "~/.clawdbrain/logs/clawdbrain.log" },
        hooks: {
          path: "~/.clawdbrain/hooks.json5",
        ...
        channels: {
          telegram: {
            accounts: {
              personal: {
                tokenFile: "~/.clawdbrain/telegram.token",
              },
            },
          },
          imessage: {
      ...
      expect(cfg.logging?.file).toBe(path.join(home, ".clawdbrain", "logs", "clawdbrain.log"));
      expect(cfg.hooks?.path).toBe(path.join(home, ".clawdbrain", "hooks.json5"));
      ...
      expect(cfg.channels?.telegram?.accounts?.personal?.tokenFile).toBe(
        path.join(home, ".clawdbrain", "telegram.token"),
      );
