...
async function withTempHome(run: (home: string) => Promise<void>): Promise<void> {
  const home = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-config-"));
  try {
    await run(home);
  } finally {
...
async function writeConfig(
  home: string,
  dirname: ".clawdbrain" | ".moltbot" | ".clawdbot",
  port: number,
) {
  const dir = path.join(home, dirname);
  await fs.mkdir(dir, { recursive: true });
  const configPath = path.join(dir, "clawdbrain.json");
  await fs.writeFile(configPath, JSON.stringify({ gateway: { port } }, null, 2));
  return configPath;
}

describe("config io compat (new + legacy folders)", () => {
  it("prefers ~/.clawdbrain/clawdbrain.json when both configs exist", async () => {
    await withTempHome(async (home) => {
      const newConfigPath = await writeConfig(home, ".clawdbrain", 19001);
  ...
  it("falls back to ~/.clawdbot/clawdbrain.json when only oldest legacy exists", async () => {
    await withTempHome(async (home) => {
  ...
  it("honors explicit legacy config path env override", async () => {
    await withTempHome(async (home) => {
      const newConfigPath = await writeConfig(home, ".clawdbrain", 19002);
