...
describe("sessions", () => {
  ...
  it("derives session transcripts dir from CLAWDBRAIN_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { CLAWDBRAIN_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("includes topic ids in session transcript filenames", () => {
    ...
    process.env.CLAWDBRAIN_STATE_DIR = "/custom/state";
    try {
