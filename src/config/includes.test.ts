...
const ETC_CLAWDBRAIN_DIR = path.join(ROOT_DIR, "etc", "clawdbrain");
const SHARED_DIR = path.join(ROOT_DIR, "shared");

const DEFAULT_BASE_PATH = path.join(CONFIG_DIR, "clawdbrain.json");

function configPath(...parts: string[]) {
  return path.join(CONFIG_DIR, ...parts);
}

function etcClawdbrainPath(...parts: string[]) {
  return path.join(ETC_CLAWDBRAIN_DIR, ...parts);
}
...
describe("resolveConfigIncludes", () => {
  ...
  it("resolves absolute path $include", () => {
    const absolute = etcClawdbrainPath("agents.json");
  ...
  it("resolves parent directory references", () => {
    ...
    expect(resolve(obj, files, configPath("sub", "clawdbrain.json"))).toEqual({
      shared: true,
    });
  });
});
