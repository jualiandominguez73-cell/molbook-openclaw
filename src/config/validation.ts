...
import type { ClawdbrainConfig, ConfigValidationIssue } from "./types.js";
...
function validateIdentityAvatar(config: ClawdbrainConfig): ConfigValidationIssue[] {
...
export function validateConfigObject(
  raw: unknown,
): { ok: true; config: ClawdbrainConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = ClawdbrainSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as ClawdbrainConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as ClawdbrainConfig);
  ...
  return {
    ...
    config: applyModelDefaults(
      applyAgentDefaults(applySessionDefaults(validated.data as ClawdbrainConfig)),
    ),
  };
}
