...
describe("config warning deduplication", () => {
  ...
  it("re-logs warnings when the warning content changes", async () => {
    await withTempHome(async (home) => {
      ...
      // First config: discord disabled with config
      await fs.writeFile(
        configPath,
        JSON.stringify({
      ...
      // Change config: now telegram is disabled with config instead
      await fs.writeFile(
        configPath,
        JSON.stringify({
          plugins: { entries: { telegram: { enabled: false, config: {} } } },
        }),
      );
      ...
      expect(configWarnings[1]).toContain("telegram");
    });
  });
});
