import type { ClawdbrainConfig } from "./config.js";
import {
  getChatChannelMeta,
  listChatChannels,
...
function isTelegramConfigured(cfg: ClawdbrainConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: ClawdbrainConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: ClawdbrainConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isSignalConfigured(cfg: ClawdbrainConfig): boolean {
...
function isIMessageConfigured(cfg: ClawdbrainConfig): boolean {
...
function isWhatsAppConfigured(cfg: ClawdbrainConfig): boolean {
...
function isGenericChannelConfigured(cfg: ClawdbrainConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: ClawdbrainConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: ClawdbrainConfig, providerId: string): boolean {
...
function isPluginExplicitlyDisabled(cfg: ClawdbrainConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: ClawdbrainConfig, pluginId: string): boolean {
...
function enablePluginEntry(cfg: ClawdbrainConfig, pluginId: string): ClawdbrainConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
