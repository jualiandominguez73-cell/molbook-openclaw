...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      (window.webkit.messageHandlers.clawdbrainCanvasA2UIAction ||
        window.webkit.messageHandlers.clawdbrainCanvasA2UIAction)
    );
  const hasAndroid = () =>
    !!(
      (window.clawdbrainCanvasA2UIAction &&
        typeof window.clawdbrainCanvasA2UIAction.postMessage === "function") ||
      (window.clawdbrainCanvasA2UIAction &&
        typeof window.clawdbrainCanvasA2UIAction.postMessage === "function")
    );
  const legacySend = typeof window.clawdbrainSendUserAction === "function" ? window.clawdbrainSendUserAction : undefined;
  if (!window.clawdbrainSendUserAction && legacySend) {
    window.clawdbrainSendUserAction = legacySend;
  }
  if (!window.clawdbrainSendUserAction && typeof window.clawdbrainSendUserAction === "function") {
    window.clawdbrainSendUserAction = window.clawdbrainSendUserAction;
  }
  const hasHelper = () =>
    typeof window.clawdbrainSendUserAction === "function" ||
    typeof window.clawdbrainSendUserAction === "function";
  statusEl.innerHTML =
    "Bridge: " +
  ...
  window.addEventListener("clawdbrain:a2ui-action-status", (ev) => {
  ...
  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android Clawdbrain node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.clawdbrainSendUserAction === "function"
        ? window.clawdbrainSendUserAction
        : window.clawdbrainSendUserAction;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.CLAWDBRAIN_SKIP_CANVAS_HOST)) return true;
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>Clawdbrain Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
