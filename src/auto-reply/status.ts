...
type AgentConfig = Partial<NonNullable<NonNullable<ClawdbrainConfig["agents"]>["defaults"]>>;
...
const readUsageFromSessionLog = (
  ...
  | undefined => {
  // Transcripts are stored at the session file path (fallback: ~/.clawdbrain/sessions/<SessionId>.jsonl)
...
export function buildStatusMessage(args: StatusArgs): string {
  ...
  const versionLine = `🦞 Clawdbrain ${VERSION}${commit ? ` (${commit})` : ""}`;
...
export function buildHelpMessage(cfg?: ClawdbrainConfig): string {
  const lines = ["ℹ️ Help", ""];
