...
import { scheduleGatewaySigusr1Restart, triggerClawdbrainRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting clawdbrain in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerClawdbrainRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting clawdbrain via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
