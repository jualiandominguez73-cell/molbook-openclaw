...
export function isCommandEnabled(cfg: ClawdbrainConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: ClawdbrainConfig): {
  provider: string;
  model: string;
} {
...
export function getCommandDetection(_cfg?: ClawdbrainConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: ClawdbrainConfig) {
