...
async function noteZalouserHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "",
      "Docs: https://docs.clawdbrain.bot/channels/zalouser",
    ].join("\n"),
    "Zalo Personal Setup",
  );
}
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  getCurrent: (cfg) => ((cfg as ClawdbrainConfig).channels?.zalouser?.dmPolicy ?? "pairing") as "pairing",
  setPolicy: (cfg, policy) => setZalouserDmPolicy(cfg as ClawdbrainConfig, policy),
  promptAllowFrom: async ({ cfg, prompter, accountId }) => {
    const id =
      accountId && normalizeAccountId(accountId)
        ? normalizeAccountId(accountId) ?? DEFAULT_ACCOUNT_ID
        : resolveDefaultZalouserAccountId(cfg as ClawdbrainConfig);
...
export const zalouserOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  dmPolicy,
  getStatus: async ({ cfg }) => {
    const ids = listZalouserAccountIds(cfg as ClawdbrainConfig);
  ...
  configure: async ({ cfg, prompter, accountOverrides, shouldPromptAccountIds, forceAllowFrom }) => {
    ...
    const defaultAccountId = resolveDefaultZalouserAccountId(cfg as ClawdbrainConfig);
    ...
    let next = cfg as ClawdbrainConfig;
