...
- This plugin integrates Lobster with Clawdbrain *without core changes*.
...
## Using `clawd.invoke` (Lobster → Clawdbrain tools)

Some Lobster pipelines may include a `clawd.invoke` step to call back into Clawdbrain tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).

For this to work, the Clawdbrain Gateway must expose the tool bridge endpoint and the target tool must be allowed by policy:

- Clawdbrain provides an HTTP endpoint: `POST /tools/invoke`.
- The request is gated by **gateway auth** (e.g. `Authorization: Bearer …` when token auth is enabled).
- The invoked tool is gated by **tool policy** (global + per-agent + provider + group policy). If the tool is not allowed, Clawdbrain returns `404 Tool not available`.
