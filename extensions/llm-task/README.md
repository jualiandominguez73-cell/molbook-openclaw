...
`clawd.invoke --each`) without adding new Clawdbrain code per workflow.
...
This extension depends on Clawdbrain internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Clawdbrain extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.clawdbrain/extensions` as a standalone plugin directory.
