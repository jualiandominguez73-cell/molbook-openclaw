...
// NOTE: This extension is intended to be bundled with Clawdbrain.
// When running from source (tests/dev), Clawdbrain internals live under src/.
...
export function createLlmTaskTool(api: ClawdbrainPluginApi) {
  return {
    name: "llm-task",
    description:
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      try {
        tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-llm-task-"));
