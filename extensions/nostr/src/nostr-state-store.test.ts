...
import type { PluginRuntime } from "clawdbrain/plugin-sdk";
...
async function withTempStateDir<T>(fn: (dir: string) => Promise<T>) {
  const previous = process.env.CLAWDBRAIN_STATE_DIR;
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "clawdbrain-nostr-"));
  process.env.CLAWDBRAIN_STATE_DIR = dir;
  setNostrRuntime({
    state: {
      resolveStateDir: (env, homedir) => {
        const override = env.CLAWDBRAIN_STATE_DIR?.trim();
        if (override) return override;
        return path.join(homedir(), ".clawdbrain");
      },
    },
  } as PluginRuntime);
  try {
    ...
    if (previous === undefined) delete process.env.CLAWDBRAIN_STATE_DIR;
    else process.env.CLAWDBRAIN_STATE_DIR = previous;
