...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: CLAWDBRAIN_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: ClawdbrainConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var CLAWDBRAIN_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
const dmPolicy: ChannelOnboardingDmPolicy = {
  ...
  setPolicy: (cfg, policy) => {
    ...
    return setTwitchAccessControl(cfg as ClawdbrainConfig, allowedRoles, true);
  },
  promptAllowFrom: async ({ cfg, prompter }) => {
    ...
    return setTwitchAccount(cfg as ClawdbrainConfig, {
      ...(account ?? undefined),
      allowFrom,
    });
  },
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.CLAWDBRAIN_TWITCH_ACCESS_TOKEN?.trim();
