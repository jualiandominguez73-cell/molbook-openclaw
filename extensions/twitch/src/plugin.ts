/**
 * Twitch channel plugin for Clawdbrain.
 *
...
import { buildChannelConfigSchema } from "clawdbrain/plugin-sdk";
 ...
 * for Clawdbrain. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: ClawdbrainConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: ClawdbrainConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: ClawdbrainConfig): boolean => {
