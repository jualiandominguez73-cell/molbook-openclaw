import type { ChannelAccountSnapshot, ChannelPlugin } from "../../../src/channels/plugins/types.js";
...
export const bluebubblesPlugin: ChannelPlugin<ResolvedBlueBubblesAccount> = {
  ...
  config: {
    listAccountIds: (cfg) => listBlueBubblesAccountIds(cfg as CoreConfig),
    resolveAccount: (cfg, accountId) => resolveBlueBubblesAccount({ cfg: cfg as CoreConfig, accountId }),
    defaultAccountId: (cfg) => resolveDefaultBlueBubblesAccountId(cfg as CoreConfig),
    ...
    resolveAllowFrom: ({ cfg, accountId }) =>
      (resolveBlueBubblesAccount({ cfg: cfg as CoreConfig, accountId }).config.allowFrom ?? []).map(
        (entry) => String(entry),
      ),
  ...
  security: {
    resolveDmPolicy: ({ cfg, accountId, account }) => {
      ...
      const useAccountPath = Boolean(
        (cfg as CoreConfig).channels?.bluebubbles?.accounts?.[resolvedAccountId],
      );
