import { createActionGate, jsonResult, readNumberParam, readReactionParams, readStringParam } from "../../../src/agents/tools/common.js";
import type {
...
import type { BlueBubblesSendTarget, CoreConfig } from "./types.js";
...
export const bluebubblesMessageActions: ChannelMessageActionAdapter = {
  listActions: ({ cfg }) => {
    const account = resolveBlueBubblesAccount({ cfg: cfg as CoreConfig });
    if (!account.enabled || !account.configured) return [];
    const gate = createActionGate((cfg as CoreConfig).channels?.bluebubbles?.actions);
  ...
  handleAction: async ({ action, params, cfg, accountId }) => {
    ...
    const account = resolveBlueBubblesAccount({ cfg: cfg as CoreConfig, accountId: accountId ?? undefined });
