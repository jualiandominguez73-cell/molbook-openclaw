import { normalizeBlueBubblesServerUrl, type BlueBubblesAccountConfig, type CoreConfig } from "./types.js";
...
export function listBlueBubblesAccountIds(cfg: CoreConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: CoreConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: CoreConfig): ResolvedBlueBubblesAccount[] {
