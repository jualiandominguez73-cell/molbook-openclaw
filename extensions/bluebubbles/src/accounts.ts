...
export function listBlueBubblesAccountIds(cfg: ClawdbrainConfig): string[] {
  const ids = listConfiguredAccountIds(cfg);
  if (ids.length === 0) return [DEFAULT_ACCOUNT_ID];
  return ids.sort((a, b) => a.localeCompare(b));
}

export function resolveDefaultBlueBubblesAccountId(cfg: ClawdbrainConfig): string {
...
export function listEnabledBlueBubblesAccounts(cfg: ClawdbrainConfig): ResolvedBlueBubblesAccount[] {
