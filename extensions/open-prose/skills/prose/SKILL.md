...
metadata: {"clawdbrain":{"emoji":"🪶","homepage":"https://www.prose.md"}}
---
...
- **Task tool** in the upstream spec == Clawdbrain `sessions_spawn`
- **File I/O** == Clawdbrain `read`/`write`
- **Remote fetch** == Clawdbrain `web_fetch` (or `exec` with curl when POST is required)
