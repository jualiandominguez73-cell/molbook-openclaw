...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["clawdbrainPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["clawdbrain", "pollId"]) ??
    readNestedString(value, ["clawdbrain", "poll", "id"]) ??
    readNestedString(value, ["data", "clawdbrainPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "clawdbrain", "pollId"]);
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          clawdbrainPollId: pollId,
        },
        msteams: {
          type: "messageBack",
          text: "clawdbrain poll vote",
          displayText: "Vote recorded",
          value: { clawdbrainPollId: pollId },
        },
      },
    ],
  };
