...
export function renderCron(props: CronProps) {
  ...
  return html`
    ...
    <section class="card" style="margin-top: 18px;">
      ...
      <div class="card-sub">网关中存储的所有调度任务。</div>
      ${props.jobs.length === 0
  ...
  return html`
    <div class=${itemClass} @click=${() => props.onLoadRuns(job.id)}>
      <div class="list-main">
        ...
        ${job.agentId ? html`<div class="muted">智能体: ${job.agentId}</div>` : nothing}
        <div class="chip-row" style="margin-top: 6px;">
          <span class="chip">${job.enabled ? "已启用" : "已禁用"}</span>
          <span class="chip">${job.sessionTarget === "main" ? "主会话" : job.sessionTarget === "isolated" ? "隔离会话" : job.sessionTarget}</span>
          <span class="chip">${job.wakeMode === "next-heartbeat" ? "下次心跳" : job.wakeMode === "now" ? "立即" : job.wakeMode}</span>
        </div>
      </div>
      <div class="list-meta">
