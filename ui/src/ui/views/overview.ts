...
export function renderOverview(props: OverviewProps) {
  const snapshot = props.hello?.snapshot as
    ...
    | undefined;
  const uptime = snapshot?.uptimeMs ? formatDurationMs(snapshot.uptimeMs) : "未知";
  ...
  const authHint = (() => {
      ...
      return html`
        <div class="muted" style="margin-top: 8px;">
          ...
          <div style="margin-top: 6px;">
            <span class="mono">openclaw dashboard --no-open</span> → 令牌化 URL<br />
    ...
    return html`
      <div class="muted" style="margin-top: 8px;">
        验证失败。请使用 <span class="mono">openclaw dashboard --no-open</span> 重新复制令牌化 URL，或更新令牌，
        然后点击连接。
        <div style="margin-top: 6px;">
          <a
            class="session-link"
  ...
  const insecureContextHint = (() => {
    ...
    return html`
      <div class="muted" style="margin-top: 8px;">
        此页面为 HTTP，因此浏览器阻止了设备身份验证。请使用 HTTPS (Tailscale Serve) 或在网关主机上打开
  ...
  return html`
    <section class="grid grid-cols-2">
      <div class="card">
        ...
        <div class="card-sub">控制台连接的位置及其验证方式。</div>
    ...
    <section class="grid grid-cols-3" style="margin-top: 18px;">
      ...
      <div class="card stat-card">
        ...
        <div class="stat-value">${props.sessionsCount ?? "未知"}</div>
    ...
    <section class="card" style="margin-top: 18px;">
      ...
      <div class="note-grid" style="margin-top: 14px;">
        <div>
          ...
          <div class="muted">
            首选 serve 模式，以保持网关在回环地址并通过 tailnet 验证。
          </div>
        </div>
        <div>
        ...
        <div>
          ...
          <div class="muted">为重复运行的任务使用独立会话。</div>
        </div>
      </div>
    </section>
  `;
