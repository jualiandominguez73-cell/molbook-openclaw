...
struct IOSGatewayChatTransport: ClawdbrainChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ClawdbrainChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(ClawdbrainChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> ClawdbrainChatHistoryPayload {
        ...
        return try JSONDecoder().decode(ClawdbrainChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [ClawdbrainChatAttachmentPayload]) async throws -> ClawdbrainChatSendResponse
    {
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [ClawdbrainChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        return try JSONDecoder().decode(ClawdbrainChatSendResponse.self, from: res)
    }

    func requestHealth(timeoutMs: Int) async throws -> Bool {
        let seconds = max(1, Int(ceil(Double(timeoutMs) / 1000.0)))
        let res = try await self.gateway.request(method: "health", paramsJSON: nil, timeoutSeconds: seconds)
        return (try? JSONDecoder().decode(ClawdbrainGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ClawdbrainGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ClawdbrainChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: ClawdbrainAgentEventPayload.self)
                        {
