...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: ClawdbrainProtocol.AnyCodable(["ok": ClawdbrainProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: ClawdbrainProtocol.AnyCodable(["ok": ClawdbrainProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = ClawdbrainProtocol.AnyCodable([
            "runId": ClawdbrainProtocol.AnyCodable("run-1"),
            "sessionKey": ClawdbrainProtocol.AnyCodable("main"),
            "state": ClawdbrainProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: ClawdbrainProtocol.AnyCodable(["a": ClawdbrainProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
