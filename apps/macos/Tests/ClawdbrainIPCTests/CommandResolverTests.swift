...
@Suite(.serialized) struct CommandResolverTests {
    ...
    @Test func prefersClawdbrainBinary() async throws {
        ...
        let clawdbrainPath = tmp.appendingPathComponent("node_modules/.bin/clawdbrain")
        try self.makeExec(at: clawdbrainPath)

        let cmd = CommandResolver.clawdbrainCommand(subcommand: "gateway", defaults: defaults, configRoot: [:])
        #expect(cmd.prefix(2).elementsEqual([clawdbrainPath.path, "gateway"]))
    }
    ...
    @Test func fallsBackToNodeAndScript() async throws {
        ...
        let scriptPath = tmp.appendingPathComponent("bin/clawdbrain.js")
        try self.makeExec(at: nodePath)
        try "#!/bin/sh\necho v22.0.0\n".write(to: nodePath, atomically: true, encoding: .utf8)
        try FileManager().setAttributes([.posixPermissions: 0o755], ofItemAtPath: nodePath.path)
        try self.makeExec(at: scriptPath)

        let cmd = CommandResolver.clawdbrainCommand(
            subcommand: "rpc",
            defaults: defaults,
            configRoot: [:],
    ...
    @Test func fallsBackToPnpm() async throws {
        ...
        let cmd = CommandResolver.clawdbrainCommand(subcommand: "rpc", defaults: defaults, configRoot: [:])

        #expect(cmd.prefix(4).elementsEqual([pnpmPath.path, "--silent", "clawdbrain", "rpc"]))
    }
    ...
    @Test func pnpmKeepsExtraArgsAfterSubcommand() async throws {
        ...
        #expect(cmd.prefix(5).elementsEqual([pnpmPath.path, "--silent", "clawdbrain", "health", "--json"]))
    ...
    @Test func buildsSSHCommandForRemoteMode() async throws {
        ...
        defaults.set("/srv/clawdbrain", forKey: remoteProjectRootKey)
        ...
        if let script = cmd.last {
            #expect(script.contains("PRJ='/srv/clawdbrain'"))
            #expect(script.contains("cd \"$PRJ\""))
            #expect(script.contains("clawdbrain"))
    ...
    @Test func configRootLocalOverridesRemoteDefaults() async throws {
        ...
        #expect(cmd.first == clawdbrainPath.path)
        #expect(cmd.count >= 2)
        if cmd.count >= 2 {
            #expect(cmd[1] == "daemon")
        }
