...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: ClawdbrainProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: ClawdbrainProtocol.AnyCodable] = [
            "presence": .init([ClawdbrainProtocol.AnyCodable(entry)]),
        ]
        let payload = ClawdbrainProtocol.AnyCodable(payloadMap)
