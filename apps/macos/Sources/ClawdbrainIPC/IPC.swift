...
public struct CanvasShowResult: Codable, Sendable {
    /// Session directory on disk (e.g. `~/Library/Application Support/Clawdbrain/canvas/<session>/`).
...
public let controlSocketPath = FileManager()
    ...
    .appendingPathComponent("Library/Application Support/clawdbrain/control.sock")
    .path
