...
enum CommandResolver {
    private static let projectRootDefaultsKey = "clawdbrain.gatewayProjectRootPath"
    private static let helperName = "clawdbrain"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let binEntry = root.appendingPathComponent("bin/clawdbrain.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/clawdbrain")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let clawdbrainPaths = self.clawdbrainManagedPaths(home: home)
        if !clawdbrainPaths.isEmpty {
            extras.insert(contentsOf: clawdbrainPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + clawdbrainPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func clawdbrainManagedPaths(home: URL) -> [String] {
        let base = home.appendingPathComponent(".clawdbrain")
    ...
    static func clawdbrainExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectClawdbrainExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        let candidate = self.projectRoot().appendingPathComponent("bin/clawdbrain.js").path
        return FileManager().isReadableFile(atPath: candidate) ? candidate : nil
    }

    static func hasAnyClawdbrainInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.clawdbrainExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func clawdbrainNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            ...
            if let clawdbrainPath = self.projectClawdbrainExecutable(projectRoot: root) {
                return [clawdbrainPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "clawdbrain", subcommand] + extraArgs
            }
            if let clawdbrainPath = self.clawdbrainExecutable(searchPaths: searchPaths) {
                return [clawdbrainPath, subcommand] + extraArgs
            }

            let missingEntry = """
            clawdbrain entrypoint missing (looked for dist/index.js or bin/clawdbrain.js); run pnpm build.
            """
    ...
    // Existing callers still refer to clawdbrainCommand; keep it as node alias.
    static func clawdbrainCommand(
        subcommand: String,
        extraArgs: [String] = [],
        defaults: UserDefaults = .standard,
        configRoot: [String: Any]? = nil,
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real clawdbrain CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/clawdbrain"
        ...
        \(cliSection)
        \(projectSection)
        if command -v clawdbrain >/dev/null 2>&1; then
          CLI="$(command -v clawdbrain)"
          clawdbrain \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/clawdbrain.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/clawdbrain.js"
            node "$PRJ/bin/clawdbrain.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent clawdbrain"
          pnpm --silent clawdbrain \(quotedArgs);
        else
          echo "clawdbrain CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? ClawdbrainConfigFile.loadDict()
