...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: ClawdbrainProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "com.clawdbrain", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: ClawdbrainKit.AnyCodable]()) {
                $0[$1.key] = ClawdbrainKit.AnyCodable($1.value.base)
            }
    ...
    private func friendlyGatewayMessage(_ error: Error) -> String {
           ...
           urlErr.code == .dataNotAllowed // used for WS close 1008 auth failures
        {
            ...
            let tokenKey = CommandResolver.connectionModeIsRemote()
                ...
                : "gateway.auth.token"
            return
                "Gateway rejected token; set \(tokenKey) (or CLAWDBRAIN_GATEWAY_TOKEN) " +
    ...
    private static func bridgeToProtocolArgs(
        _ value: ClawdbrainProtocol.AnyCodable?) -> [String: ClawdbrainProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: ClawdbrainProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: ClawdbrainKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: ClawdbrainProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("clawdbrain.control.heartbeat")
    static let controlAgentEvent = Notification.Name("clawdbrain.control.agent")
}
