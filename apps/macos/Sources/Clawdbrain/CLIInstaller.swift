...
enum CLIInstaller {
        ...
        fileManager: FileManager) -> String?
    {
        for basePath in searchPaths {
            let candidate = URL(fileURLWithPath: basePath).appendingPathComponent("clawdbrain").path
    ...
    static func install(statusHandler: @escaping @MainActor @Sendable (String) async -> Void) async {
        ...
        await statusHandler("Installing clawdbrain CLI…")
        ...
        if response.success {
            ...
            let summary = installedVersion.map { "Installed clawdbrain \($0)." } ?? "Installed clawdbrain."
    ...
    private static func installScriptCommand(version: String, prefix: String) -> [String] {
        ...
        let script = """
        curl -fsSL https://clawdbrain.bot/install-cli.sh | \
