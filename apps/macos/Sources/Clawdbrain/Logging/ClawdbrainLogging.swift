...
enum ClawdbrainLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = ClawdbrainOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = ClawdbrainFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        ClawdbrainLogging.bootstrapIfNeeded()
        let label = ClawdbrainLogging.makeLabel(subsystem: subsystem, category: category)
...
struct ClawdbrainOSLogHandler: LogHandler {
...
struct ClawdbrainFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = ClawdbrainLogging.parseLabel(self.label)
