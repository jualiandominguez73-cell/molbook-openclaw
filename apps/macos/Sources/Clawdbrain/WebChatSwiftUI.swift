...
private let webChatSwiftLogger = Logger(subsystem: "com.clawdbrain", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: ClawdbrainChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> ClawdbrainChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(ClawdbrainChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> ClawdbrainChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                ClawdbrainGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "chat":
                ...
                guard let chat = try? JSONDecoder().decode(
                    ClawdbrainChatEventPayload.self,
            ...
            case "agent":
                ...
                guard let agent = try? JSONDecoder().decode(
                    ClawdbrainAgentEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<ClawdbrainChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any ClawdbrainChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = ClawdbrainChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: ClawdbrainChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<ClawdbrainChatView>) -> NSViewController
    {
