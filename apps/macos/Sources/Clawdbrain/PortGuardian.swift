...
actor PortGuardian {
    ...
    private let logger = Logger(subsystem: "com.clawdbrain", category: "portguard")
    private nonisolated static let appSupportDir: URL = {
        let base = FileManager().urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        return base.appendingPathComponent("Clawdbrain", isDirectory: true)
    }()
        ...
        tunnelHealthy: Bool?) -> PortReport
    {
        ...
        let expectedCommands = ["node", "clawdbrain", "tsx", "pnpm", "bun"]
    ...
    private func isExpected(_ listener: Listener, port: Int, mode: AppState.ConnectionMode) -> Bool {
        ...
        case .remote:
            ...
            return false
        case .local:
            // The gateway daemon may listen as `clawdbrain` or as its runtime (`node`, `bun`, etc).
            if full.contains("gateway-daemon") { return true }
            // If args are unavailable, treat a clawdbrain listener as expected.
            if cmd.contains("clawdbrain"), full == cmd { return true }
            return false
        case .unconfigured:
            return false
        }
