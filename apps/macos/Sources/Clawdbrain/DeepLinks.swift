...
private let deepLinkLogger = Logger(subsystem: "com.clawdbrain", category: "DeepLink")
...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            self.presentAlert(title: "Clawdbrain is paused", message: "Unpause Clawdbrain to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run Clawdbrain agent?", message: body) else { return }
        }
