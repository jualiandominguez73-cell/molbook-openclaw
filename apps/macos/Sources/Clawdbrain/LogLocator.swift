...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["CLAWDBRAIN_LOG_DIR"], !override.isEmpty {
            return URL(fileURLWithPath: override)
        }

        return URL(fileURLWithPath: "/tmp/clawdbrain")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("clawdbrain-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("clawdbrain-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/clawdbrain/ (rolling or stdout), or nil if none exist.
    static func bestLogFile() -> URL? {
        ...
        return files
            .filter { $0.lastPathComponent.hasPrefix("clawdbrain") && $0.pathExtension == "log" }
            .max { lhs, rhs in
