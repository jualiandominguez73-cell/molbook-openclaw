...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.clawdbrain/clawdbrain.json. Prefer CLAWDBRAIN_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.clawdbrain/clawdbrain.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.clawdbrain/clawdbrain.json. Restart the gateway to apply."
        }
