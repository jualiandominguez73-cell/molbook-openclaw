...
struct PermissionsSettings: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("Allow these so Clawdbrain can notify and capture when needed.")
                .padding(.top, 4)
...
private struct LocationAccessSettings: View {
    @AppStorage(locationModeKey) private var locationModeRaw: String = ClawdbrainLocationMode.off.rawValue
    @AppStorage(locationPreciseKey) private var locationPreciseEnabled: Bool = true
    @State private var lastLocationModeRaw: String = ClawdbrainLocationMode.off.rawValue
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 6) {
            ...
            Picker("", selection: self.$locationModeRaw) {
                Text("Off").tag(ClawdbrainLocationMode.off.rawValue)
                Text("While Using").tag(ClawdbrainLocationMode.whileUsing.rawValue)
                Text("Always").tag(ClawdbrainLocationMode.always.rawValue)
            }
            .labelsHidden()
            .pickerStyle(.menu)
        ...
        .onChange(of: self.locationModeRaw) { _, newValue in
            ...
            guard let mode = ClawdbrainLocationMode(rawValue: newValue) else { return }
            Task {
    ...
    private var locationMode: ClawdbrainLocationMode {
        ClawdbrainLocationMode(rawValue: self.locationModeRaw) ?? .off
    }

    private func requestLocationAuthorization(mode: ClawdbrainLocationMode) async -> Bool {
