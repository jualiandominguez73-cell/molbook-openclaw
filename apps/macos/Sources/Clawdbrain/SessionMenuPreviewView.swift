...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "com.clawdbrain", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> ClawdbrainSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: ClawdbrainSessionPreviewEntry,
    ...
    private static func cache(payload: ClawdbrainSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: ClawdbrainChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [ClawdbrainKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [ClawdbrainKit.AnyCodable]) -> [ClawdbrainChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(ClawdbrainChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: ClawdbrainChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: ClawdbrainChatMessage) -> Bool {
    ...
    private static func toolNames(for message: ClawdbrainChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: ClawdbrainChatMessage) -> String? {
