...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: ClawdbrainOAuthStore.AnthropicOAuthStatus = ClawdbrainOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: ClawdbrainOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = ClawdbrainOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = ClawdbrainOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try ClawdbrainOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. Clawdbrain can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: ClawdbrainOAuthStore.AnthropicOAuthStatus,
