...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "com.clawdbrain", category: "mac-node")
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: ClawdbrainNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    private func currentCaps() -> [String] {
        var caps: [String] = [ClawdbrainCapability.canvas.rawValue, ClawdbrainCapability.screen.rawValue]
        if UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false {
            caps.append(ClawdbrainCapability.camera.rawValue)
        }
        let rawLocationMode = UserDefaults.standard.string(forKey: locationModeKey) ?? "off"
        if ClawdbrainLocationMode(rawValue: rawLocationMode) != .off {
            caps.append(ClawdbrainCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCommands(caps: [String]) -> [String] {
        ...
        if capsSet.contains(ClawdbrainCapability.camera.rawValue) {
        ...
        if capsSet.contains(ClawdbrainCapability.location.rawValue) {
