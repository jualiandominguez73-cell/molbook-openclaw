...
struct ConnectOptions {
    ...
    var clientId: String = "clawdbrain-macos"
...
func runConnect(_ args: [String]) async {
    ...
    if opts.help {
        print("""
        clawdbrain-mac connect

        Usage:
          clawdbrain-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
        ...
        Options:
          ...
          --json             Emit JSON
          --client-id <id>   Override client id (default: clawdbrain-macos)
    ...
    do {
        ...
        let displayName = opts.displayName ?? Host.current().localizedName ?? "Clawdbrain macOS Debug CLI"
...
private func printConnectOutput(_ output: ConnectOutput, json: Bool) {
    ...
    print("Clawdbrain macOS Gateway Connect")
...
private func resolvedToken(opts: ConnectOptions, mode: String, config: GatewayConfig) -> String? {
    ...
    if let token = ProcessInfo.processInfo.environment["CLAWDBRAIN_GATEWAY_TOKEN"], !token.isEmpty {
        return token
    }
    if mode == "remote" {
...
private func resolvedPassword(opts: ConnectOptions, mode: String, config: GatewayConfig) -> String? {
    ...
    if let password = ProcessInfo.processInfo.environment["CLAWDBRAIN_GATEWAY_PASSWORD"], !password.isEmpty {
        return password
    }
    if mode == "remote" {
