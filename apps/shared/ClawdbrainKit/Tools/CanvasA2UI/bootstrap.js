...
import "@a2ui/lit/ui";
import { themeContext } from "@clawdbrain/a2ui-theme-context";
...
class ClawdbrainA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.clawdbrainCanvasA2UIAction ??
      globalThis.webkit?.messageHandlers?.clawdbrainCanvasA2UIAction ??
      globalThis.clawdbrainCanvasA2UIAction ??
      globalThis.clawdbrainCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (
          handler === globalThis.clawdbrainCanvasA2UIAction ||
          handler === globalThis.clawdbrainCanvasA2UIAction
        ) {
