...
import Foundation
import Testing
@testable import ClawdbrainChatUI
...
private final class TestChatTransport: @unchecked Sendable, ClawdbrainChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [ClawdbrainChatHistoryPayload]
    private let sessionsResponses: [ClawdbrainChatSessionsListResponse]

    private let stream: AsyncStream<ClawdbrainChatTransportEvent>
    private let continuation: AsyncStream<ClawdbrainChatTransportEvent>.Continuation

    init(
        historyResponses: [ClawdbrainChatHistoryPayload],
        sessionsResponses: [ClawdbrainChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<ClawdbrainChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> ClawdbrainChatHistoryPayload {
        ...
        return self.historyResponses.last ?? ClawdbrainChatHistoryPayload(
        ...
        attachments _: [ClawdbrainChatAttachmentPayload]) async throws -> ClawdbrainChatSendResponse
    {
        ...
        return ClawdbrainChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> ClawdbrainChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? ClawdbrainChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: ClawdbrainChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = ClawdbrainChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = ClawdbrainChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { ClawdbrainChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = ClawdbrainChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = ClawdbrainChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = ClawdbrainChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { ClawdbrainChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
