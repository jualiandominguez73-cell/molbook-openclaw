...
@MainActor
struct ClawdbrainChatComposer: View {
    @Bindable var viewModel: ClawdbrainChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(ClawdbrainChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(ClawdbrainChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \ClawdbrainPendingAttachment.id)
                { (att: ClawdbrainPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            ClawdbrainPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            Rectangle()
                .fill(ClawdbrainChatTheme.divider)
                .frame(height: 1)
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ClawdbrainChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(ClawdbrainChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(ClawdbrainChatTheme.subtleCard)
        .clipShape(Capsule())
    }
