...
struct ChatMessageBubble: View {
    let message: ClawdbrainChatMessage
    let style: ClawdbrainChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? ClawdbrainChatTheme.userText : ClawdbrainChatTheme.assistantText
    ...
    private var inlineAttachments: [ClawdbrainChatMessageContent] {
    ...
    private var toolCalls: [ClawdbrainChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [ClawdbrainChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? ClawdbrainChatTheme.userBubble
        }
        if self.style == .onboarding {
            return ClawdbrainChatTheme.onboardingAssistantBubble
        }
        return ClawdbrainChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return ClawdbrainChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: ClawdbrainChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? ClawdbrainChatTheme.userText : ClawdbrainChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: ClawdbrainChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(ClawdbrainChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(ClawdbrainChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [ClawdbrainChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: ClawdbrainChatTheme.assistantText)
            }
        }
    }
}
