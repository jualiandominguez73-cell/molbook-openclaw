...
enum ClawdbrainChatTheme {
    ...
    static let assistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("ClawdbrainChatTheme.assistantBubble"),
        dynamicProvider: resolvedAssistantBubbleColor(for:))

    static let onboardingAssistantBubbleDynamicNSColor = NSColor(
        name: NSColor.Name("ClawdbrainChatTheme.onboardingAssistantBubble"),
...
enum ClawdbrainPlatformImageFactory {
    static func image(_ image: ClawdbrainPlatformImage) -> Image {
        #if os(macOS)
        Image(nsImage: image)
        #else
        Image(uiImage: image)
