...
private let chatUILogger = Logger(subsystem: "com.clawdbrain", category: "ClawdbrainChatUI")

@MainActor
@Observable
public final class ClawdbrainChatViewModel {
    public private(set) var messages: [ClawdbrainChatMessage] = []
    ...
    public var attachments: [ClawdbrainPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [ClawdbrainChatPendingToolCall] = []
    public private(set) var sessions: [ClawdbrainChatSessionEntry] = []
    private let transport: any ClawdbrainChatTransport
    ...
    private var pendingToolCallsById: [String: ClawdbrainChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any ClawdbrainChatTransport) {
    ...
    public var sessionChoices: [ClawdbrainChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        var seen = Set<String>()
        var recent: [ClawdbrainChatSessionEntry] = []
        for entry in sorted {
        ...
        var result: [ClawdbrainChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: ClawdbrainPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [ClawdbrainChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: ClawdbrainChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func dedupeMessages(_ messages: [ClawdbrainChatMessage]) -> [ClawdbrainChatMessage] {
        var result: [ClawdbrainChatMessage] = []
    ...
    private static func dedupeKey(for message: ClawdbrainChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [ClawdbrainChatMessageContent] = [
        ...
        let encodedAttachments = self.attachments.map { att -> ClawdbrainChatAttachmentPayload in
            ClawdbrainChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> ClawdbrainChatSessionEntry {
        ClawdbrainChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: ClawdbrainChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: ClawdbrainChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: ClawdbrainAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = ClawdbrainChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> ClawdbrainPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
